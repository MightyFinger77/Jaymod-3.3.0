...
void G_DropWeapon( gentity_t *ent, weapon_t weapon )
{
	...
	if( weapon == WP_KAR98 || weapon == WP_CARBINE ) {
		ent2->delay = client->ps.ammo[BG_FindAmmoForWeapon( (weapon_t)weapAlts[weapon] )];
	} else {
		ent2->delay = 0;
	}
...
================
Pickup_Binocs
Jaybird - Custom function for adding binocs if picked up.
================
*/
int Pickup_Binocs( gentity_t *ent, gentity_t *other ) {
	if (other && other->client) {
		if(AddWeaponToPlayer(other->client, WP_BINOCULARS, 1, 0, qfalse)) {
			other->client->ps.stats[STAT_KEYS] |= ( 1 << INV_BINOCS );
		}
		other->client->pers.binocCount++;
	}
	return -1;
}
...
int Pickup_Weapon( gentity_t *ent, gentity_t *other ) {
	...
	if( ent->item->giTag == WP_AMMO ) {
		...
		// Jaybird - Don't allow ammo pickup while knifeonly is turned on.
			...
			if( primaryWeapon || 
				...
				if( primaryWeapon ) {
					...
					if (other->client->sess.playerType == PC_SOLDIER &&
						...
						if (COM_BitCheck( other->client->ps.weapons, WP_MP40)) {
							G_DropWeapon( other, WP_MP40 );
						}
						else if ( COM_BitCheck( other->client->ps.weapons, WP_THOMPSON)) {
				...
				other->client->ps.ammoclip[BG_FindClipForWeapon( (weapon_t)ent->item->giTag )] = 0;
				other->client->ps.ammo[BG_FindAmmoForWeapon( (weapon_t)ent->item->giTag )] = 0;

				if( ent->item->giTag == WP_MORTAR ) {
					other->client->ps.ammo[BG_FindClipForWeapon( (weapon_t)ent->item->giTag )] = quantity;
					...
					other->client->ps.ammoclip[BG_FindClipForWeapon( (weapon_t)ent->item->giTag )] = quantity;

					// Gordon: secondary weapon ammo
					if( ent->delay ) {
						other->client->ps.ammo[ weapAlts[ ent->item->giTag ] ] = int(ent->delay);
					}
				}
			}
		} else {
...
//	int			quantity = 0;
	...
	// do NOT let them pick up health that is either theirs or does not have an owner
	// (i.e. dropped packs on death) when poisoned
	if( G_IsPoisoned( other ) && ( !ent->parent || ent->parent == other ))
		return 0;
	...
	// Jaybird - clear poison
	//G_ResetPoisonEvents( other );
	if( G_IsPoisoned( other ) && (!ent->parent || ent->parent != other ))
		G_ResetOnePoisonEvent( other );
...
void Touch_Item( gentity_t *ent, gentity_t *other, trace_t *trace ) {
    ...
    // tjw: if someone plays dead on a medkit don't let them pick 
    //      pick it up or they will become the living dead.
    ...
    if (ent->item->giType == IT_HEALTH && other->client->sess.playerType == PC_MEDIC) {
        ...
        if ( g_medicSelfHealDelay.integer > 0 && ent->parent && ent->parent->client &&
             ent->parent->s.teamNum == other->s.teamNum &&
             (level.time - ent->spawnTime) < g_medicSelfHealDelay.integer )
        {
            return;
        }

        // do not let medic unpoison themselves
        if (ent->parent && ent->parent->client && other->s.clientNum == ent->parent->s.clientNum && G_IsPoisoned(ent))
            return;
    }
	...
	case IT_WEAPON:
		// Jaybird - Check if item is binocs
		if (ent->item->giTag == WP_BINOCULARS)
			respawn = Pickup_Binocs( ent, other );
		else
