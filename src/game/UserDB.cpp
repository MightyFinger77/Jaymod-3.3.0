...
void
UserDB::panzerTickAll( int limit )
{
    if (limit < 1)
        return;
    ...
    for ( mapGUID_t::iterator it = _mapGUID.begin(); it != end; it++ ) {
        ...
        if (user.panzerHeld) {
            user.panzerHeld = false;

            // Only advance a valid run that is already in progress.
            if (user.panzerMaps >= 0 && ++user.panzerMaps >= limit)
                user.panzerMaps = -limit;
        }
        else if (user.panzerMaps < 0) {
            user.panzerMaps++;
        }
        else {
            user.panzerMaps = 0;
