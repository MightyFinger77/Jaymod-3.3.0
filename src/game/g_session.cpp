...
void G_WriteClientSessionData( gclient_t *client, qboolean restart )
{
	...
	if( !restart || level.warmupEndTime ) {
...
void G_ReadSessionData( gclient_t *client )
{
	...
	*s = 0;
	...
	// Do not restore stale per-map stats when this load starts a new stats period.
	if(level.fResetStats) {
		G_deleteStats(client - level.clients);
	} else if(*s) {
		G_parseStats(s);
	...
	*/
	// Jaybird - let's try something that makes sense instead :)
	bool load = false;


	// Only load scores back in if this map isn't starting a new campaign
	if (g_gametype.integer == GT_WOLF_CAMPAIGN) {
		load = g_campaigns[level.currentCampaign].current != 0 && !level.newCampaign;
	}

	// Only load scores back if this isn't the first round
	if (g_gametype.integer == GT_WOLF_LMS) {
		load = (g_currentRound.integer != 0) ? qtrue : qfalse;
	}

    // Allow XP Save to override certain game types
	switch (g_gametype.integer) {
	case GT_WOLF_CAMPAIGN:
        // If the reset after campaign flag is set, do not allow override
	...
	if(g_gametype.integer == GT_WOLF_STOPWATCH && cvars::gameState.ivalue != GS_PLAYING && test) {
		G_ClientSwap(client);
	}
...
//	sess->auto_mute_time = -1;
	...
	// Restore XP Backup
    // Note: XP Backups are read ONLY for first time connectors.
    // Otherwise, XP is read from stored sessions to avoid GUID conflicts.
...
void G_InitWorldSession( void ) {
	...
	if ( g_gametype.integer != gt ) {
		...
		char *tmp = s;
		qboolean test = (g_altStopwatchMode.integer != 0 || g_currentRound.integer == 1) ? qtrue : qfalse;
