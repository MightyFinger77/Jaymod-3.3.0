...
// XP earned since the start of the current map.
#define JM_PERMAP_XP( CL )	( (CL)->ps.persistant[PERS_SCORE] - (CL)->sess.startxptotal )
...
#define CHECKSTAT_XP()															\
	...
	for( i = 0; i < level.numConnectedClients; i++ ) {							\
		...
		if( JM_PERMAP_XP( cl ) <= 0 ) {											\
			continue;															\
		}																		\
		if( !best || JM_PERMAP_XP( cl ) > JM_PERMAP_XP( best ) ) {				\
			best = cl;															\
		}																		\
	}																			\
	if( best ) { best->hasaward = qtrue; }										\
...
// Highest per-map XP rate.
#define CHECKSTAT_XPTIME2( YY )													\
	best = NULL;																\
	for( i = 0; i < level.numConnectedClients; i++ ) {							\
		gclient_t* cl = &level.clients[ level.sortedClients[ i ] ];				\
		if( cl->sess.sessionTeam == TEAM_SPECTATOR ) {							\
		...
		if( !best || (JM_PERMAP_XP(cl)/(float)(level.time - cl->YY)) > (JM_PERMAP_XP(best)/(float)(level.time - best->YY)) ) { \
			best = cl;															\
		}																		\
	}																			\
	if( best ) { best->hasaward = qtrue; }										\
...
// Highest per-map XP rate among players who qualify for the consolation award.
#define CHECKSTAT_XPTIME( YY )													\
	best = NULL;																\
	for( i = 0; i < level.numConnectedClients; i++ ) {							\
		gclient_t* cl = &level.clients[ level.sortedClients[ i ] ];				\
		if( cl->sess.sessionTeam == TEAM_SPECTATOR ) {							\
	...
	if( best ) {																\
		if( JM_PERMAP_XP(best) < 100 || best->medals || best->hasaward) {		\
			best = NULL;														\
		}																		\
	}																			\
	Q_strcat( buffer, 1024, va( ";%s; %i ", best ? best->pers.netname : "", best ? best->sess.sessionTeam : -1 ) )
...
void G_BuildEndgameStats( void ) {
	...
	*buffer = '\0';
	...
	CHECKSTAT_XPTIME2( pers.enterTime );
	CHECKSTAT1( acc );
	CHECKSTATMIN( sess.team_kills, 5 );
	CHECKSTAT_XPTIME( pers.enterTime );
