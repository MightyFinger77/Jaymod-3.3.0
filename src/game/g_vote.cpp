...
static const vote_reference_t aVoteInfo[] = {
	...
	{ 0x1ff, "generic",		 G_Generic_v,		"Question:",			" Ask a question" },
...
{
	// Jaybird - Add Shrubbot permission
	if(!vote_allow && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
...
int G_Comp_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		...
		} else if(vote_allow_comp.integer<=0 && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
...
int G_Gametype_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		...
		// Jaybird - check for shrubbot permission
		if(!vote_allow_gametype.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
			G_voteDisableMessage(ent, arg);
			G_GametypeList(ent);
...
int G_Kick_v( gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd ) {
	...
	if( arg ) {
		...
		// Jaybird - check for shrubbot permission
		if( !vote_allow_kick.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
			G_voteDisableMessage(ent, arg);
			return G_INVALID;
		} else if( G_voteDescription(ent, fRefereeCmd, dwVoteIndex) ) {
			return G_INVALID;
		...
		// Jaybird
		if( cmd::entityHasPermission( &g_entities[pid], priv::base::voteImmunity )) {
	...
	// Vote action (vote has passed)
		...
		// Kick a player
		trap_DropClient(atoi(level.voteInfo.vote_value ), "You were kicked because a vote passed to kick you.", cache::kickTime );
...
int G_Generic_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	if( arg ) {
		// Jaybird - check for shrubbot permission
		if(!vote_allow_generic.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
			G_voteDisableMessage(ent, arg);
			return(G_INVALID);
		}
	} else {
		AP( va( "cp \"Vote passed! %s\n\"", level.voteInfo.voteString ));
	}
	return G_OK;
}
...
int G_Mute_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		...
		// Jaybird - check for shrubbot permission
		if(!vote_allow_muting.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
			G_voteDisableMessage(ent, arg);
			return(G_INVALID);
	...
	// Vote action (vote has passed)
		...
		if( level.clients[pid].sess.referee != RL_RCON ) {
            G_MutePlayer(g_entities + pid, "vote");
...
int G_Map_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		...
		// Jaybird - check for shrubbot permission
		if(!vote_allow_map.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
			G_voteDisableMessage(ent, arg);
...
int G_MapRestart_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	// Vote request (vote is being initiated)
	if(arg) {
		if( !vote_allow_matchrestart.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
			G_voteDisableMessage(ent, arg);
			return(G_INVALID);
		}
		if(trap_Argc() > 2) {
...
int G_MatchReset_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		// Jaybird - check for shrubbot permission
		if(!vote_allow_matchreset.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
			G_voteDisableMessage(ent, arg);
			return(G_INVALID);
...
int G_Nextmap_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		if(trap_Argc() > 2) {
			G_refPrintf(ent, "Usage: ^3%s %s%s\n", ((fRefereeCmd) ? "\\ref" : "\\callvote"), arg, aVoteInfo[dwVoteIndex].pszVoteHelp);
			return(G_INVALID);
		} else if(!vote_allow_nextmap.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
			// Jaybird - check for shrubbot permission
			G_voteDisableMessage(ent, arg);
			return(G_INVALID);
		} else {
...
int G_Referee_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		...
		// Jaybird - check for shrubbot permission
		if(!vote_allow_referee.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
			G_voteDisableMessage(ent, arg);
			return(G_INVALID);
		}
...
int G_ShuffleTeams_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		if(trap_Argc() > 2) {
			G_refPrintf(ent, "Usage: ^3%s %s%s\n", ((fRefereeCmd) ? "\\ref" : "\\callvote"), arg, aVoteInfo[dwVoteIndex].pszVoteHelp);	// CHRUKER: b047 - Removed unneeded linebreak
			return(G_INVALID);
		// Jaybird - check for shrubbot permission
		} else if(!vote_allow_shuffleteamsxp.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission(ent, priv::base::voteAny)) {
			G_voteDisableMessage(ent, arg);
			return(G_INVALID);
		}
...
int G_StartMatch_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	// Vote request (vote is being initiated)
	if(arg) {
		if( !vote_allow_startmatch.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission( ent, priv::base::voteAny )) {
			G_voteDisableMessage(ent, arg);
			return(G_INVALID);
		}
...
int G_SwapTeams_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		if(trap_Argc() > 2) {
			...
			return(G_INVALID);
		// Jaybird - check for shrubbot permission
		} else if(!vote_allow_swapteams.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission(ent, priv::base::voteAny)) {
			G_voteDisableMessage(ent, arg);
			return(G_INVALID);
		}
...
int G_Timelimit_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		// Jaybird - check for shrubbot permission
		if(!vote_allow_timelimit.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission(ent, priv::base::voteAny)) {
			G_voteDisableMessage(ent, arg);
...
void G_WarmupDamageTypeList(gentity_t *ent)
{
	...
	for(i=0; i< int(sizeof(warmupType) / sizeof(char *)); i++) G_refPrintf(ent, "  %d ^3(%s)", i, warmupType[i]);
	G_refPrintf(ent, "\n");
}
...
int G_Warmupfire_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		...
		// Jaybird - check for shrubbot permission
		if(!vote_allow_warmupdamage.integer && ent && !ent->client->sess.referee && !cmd::entityHasPermission(ent, priv::base::voteAny)) {
			G_voteDisableMessage(ent, arg);
