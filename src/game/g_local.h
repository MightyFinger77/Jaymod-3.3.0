...
// Time required to steal a uniform, in milliseconds.
#define	STEAL_UNIFORM_TIME	2500

// Delay between repeated fire-support denial messages.
#define	FIRE_SUPPORT_DENY_DEBOUNCE	3000

// Stop thrown knives that remain trapped and bouncing.
#define	KNIFE_MAX_BOUNCES	4

// Delay between landmine-spotted announcements from one player.
#define	MINE_SPOT_ANNOUNCE_DEBOUNCE	1000

// Limits repeated attempts to mount tanks and emplaced guns.
#define	MOUNT_RATE_WINDOW		1000
#define	MOUNT_RATE_MAX			2
#define	MOUNT_RATE_MIN_INTERVAL	500
...
// LAG_MAX_COMMANDS / FRAMERATE = total lag possible before packets dropped
// 512 commands / 333 commands per second = 1.5 seconds (overkill)
#define LAG_MAX_COMMANDS                512

// Maximum milliseconds of movement in one frame
// 1s / 75ms = 13.333fps minimum for optimal operation (commands get split below threshold)
// this is only sets allowable burst time -- it's still decremented at the
// server's framerate (in realtime, not level.time)
#define LAG_MAX_DELTA                   75

// milliseconds of added lag from latest command packet before packets
// are dropped entirely
#define LAG_MAX_DROP_THRESHOLD          800

// milliseconds of added lag from latest command packet where commands
// are dropped if one reaches LAG_MAX_DROP_THRESHOLD
#define LAG_MIN_DROP_THRESHOLD          (LAG_MAX_DROP_THRESHOLD - 200)

// decay on lag, so they don't get lag permanently added, values > 1 decay
...
struct gentity_s {
	...
	int				stealTime;
	int				stealProgress;	// Milliseconds of contact accrued.
	int				clientCheckAlarm; // when client version should be checked
	int				spawnDebug;

    int landmineSkillWhenPlanted;

    int poisonGasAlarm;
    int poisonGasDamage;
    int poisonGasRadius;
    int poisonGasWeaponType; // only set when methodOfDeath == MOD_POISON_GAS

    int nextMolotovScreamTime; // earliest time next molotov scream is allowed
...
typedef struct {
	unsigned int atts;
	unsigned int subshots;
	unsigned int deaths;
	unsigned int hits;
	unsigned int kills;
	unsigned int friendlyHits;
	unsigned int totalHits;

    // Zones
    unsigned int headshots;
    unsigned int leftlegshots;
    unsigned int rightlegshots;
    unsigned int bodyshots;
    unsigned int leftarmshots;
    unsigned int rightarmshots;
} weapon_stat_t;
...
typedef struct {
	...
	// shrubbot text shortcuts
	char lastammo[MAX_NETNAME];    // Last person to give you ammo
	char lastkilled[MAX_NETNAME];  // Last person who killed you
	char lasthealth[MAX_NETNAME];  // Last person who gave you health
	char lastkill[MAX_NETNAME];    // Last person you killed
	char lastrevive[MAX_NETNAME];  // Last person who revived you
...
//	int			accuracy_hits;		// total number of hits
    ...
    int         lastkilledby_client;// last client that killed this client
	...
	int			lastFireSupportDenyTime;

	// Mount rate-limit state.
	int			mountRateWindow;
	int			mountRateCount;
	int			lastMountTime;
...
// Jaybird - increased for compatibility with bug exploits (i.e. temple)
...
typedef struct {
	...
	int			warmupEndTime;		// time which warmup ends and match should begin
...
//	int			snd_fry;				// sound index for standing in lava
	...
	int			doorAllowTeams;	// used by bots to decide whether or not to use team travel flags
...
// ===================
	...
	// Jaybird
	char		lastBloodPlayer[MAX_NETNAME];
	char		firstBloodPlayer[MAX_NETNAME];
	bool		crazyGravity;
    int         crazyGravityTime;
	int			defaultGravity; // For restoring gravity in crazy gravity

    // Longest killing spree
	int			longestSpree;
	char		longestSpreeName[MAX_NETNAME];
	...
	// Next Admin DB autosave, in trap_Milliseconds time.
	int			nextDbSaveTime;
...
// g_cmds.c
//
qboolean AddWeaponToPlayer( gclient_t*, weapon_t, int, int, qboolean );
char* ConcatArgs( int );
...
string SanitizeString(const string&, bool);
...
void G_Damage (gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, const vec3_t dir, vec3_t point, int damage, int dflags, int mod);
...
void G_killSpreeSounds(char * sound);
...
#define DAMAGE_JAY_NO_PROTECTION	0X00000080	// since DAMAGE_NO_PROTECTION didn't really work heh.
...
void G_RunFlamechunk( gentity_t *ent, int msec );
...
gentity_t* G_ThrowingKnife_launch ( gentity_t&, int );
...
// g_client.c
//
bool ClientConnect( string&, int clientNum, qboolean firstTime, qboolean isBot );
...
extern vmCvar_t		g_requireClientVersion;
extern vmCvar_t		g_dbAutoSave;
...
string::size_type trap_Argv( int, string& );
vector<string>::size_type trap_Args( vector<string>& );
...
void	trap_SendServerCommand( const string&, const text::Buffer&, int );
...
qboolean trap_GetTag( int clientNum, int tagFileNumber, char *tagName, orientation_t* );
...
void G_BouncingBettyTrip ( gentity_t* );
void G_ExplodeMissile    ( gentity_t* );
void G_PoisonGasExplode  ( gentity_t* );
void G_PoisonGasTrip     ( gentity_t* );
...
#define BODY_AMMO(ENT) ENT->s.otherEntityNum2
...
void G_CountTeamLandminesBoth ( int* axisCount, int* alliedCount );
...
#define	SE_FOFS(x) offsetof(g_serverEntity_t, x)
...
char *G_createStats(gentity_t *refEnt, bool logPrint);
void G_deleteStats(int nClient);
qboolean G_desiredFollow(gentity_t *ent, int nTeam);
void G_globalSound( const char* );
...
int G_Generic_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd);
...
#include <game/SEngine.h>

#include <game/Privilege.h>
#include <game/PrivilegeSet.h>
#include <game/Level.h>
#include <game/User.h>
#include <game/MapRecord.h>
...
#include <game/TraceContext.h>
#include <game/AbstractBulletVolume.h>
#include <game/AbstractBulletModel.h>
#include <game/AbstractHitVolume.h>
#include <game/AbstractHitModel.h>
#include <game/Client.h>
#include <game/Entity.h>
#include <game/AdminLog.h>
