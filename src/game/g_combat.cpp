...
#ifdef FEATURE_LUA
	G_LuaHook_Obituary( self - g_entities, attacker ? ( attacker - g_entities ) : ENTITYNUM_WORLD, meansOfDeath );
#endif
...
#ifdef FEATURE_LUA
	G_LuaHook_Damage( targ - g_entities, attacker - g_entities, damage, dflags, mod );
#endif

	// Discard player damage from ordnance whose owner has disconnected.
	if ( targ->client && attacker->client &&
	     attacker->client->pers.connected == CON_DISCONNECTED ) {
		return;
	}
...
//	Team_CheckHurtCarrier(targ, attacker);
	...
	if( take ) {
		...
		if( g_damagexp.integer && client && G_GetTeamFromEntity( inflictor ) != G_GetTeamFromEntity( targ )) {
			...
			if( skill >= 0 ) {
				// Award XP only for health actually removed.
				int xpDamage = take;
				if( targ->health < 0 )
					xpDamage += targ->health;
				if( xpDamage > 0 )
					G_AddSkillPoints( attacker, (skillType_t)skill, xpDamage * .02 );
			}
		}
