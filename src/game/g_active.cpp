...
bool G_AllowAntiwarp( int clientNum )
{
    ...
    // Fixed movement does not need antiwarp and its rounded times can stall the queue.
...
//		G_SendScore(ent);
	...
	if((ent->client->sess.spectatorState == SPECTATOR_FOLLOW) || (ent->client->ps.pm_flags & PMF_LIMBO)) {
		...
		if ( clientNum >= 0 ) {
			...
			// Players in limbo may only follow someone on their own team.
			if( ent->client->sess.sessionTeam != TEAM_SPECTATOR &&
			...
			if(clientNum >= 0 && cl->pers.connected == CON_CONNECTED && cl->sess.sessionTeam != TEAM_SPECTATOR) {
				...
				if(ent->client->sess.sessionTeam != TEAM_SPECTATOR && (ent->client->ps.pm_flags & PMF_LIMBO)) {
					...
					// Keep the spectator's score when copying the target state.
					int savedScore = ent->client->ps.persistant[PERS_SCORE];
					ent->client->ps = cl->ps;
					ent->client->ps.pm_flags |= PMF_FOLLOW;
