...
User::decode( map<string,string>& data )
{
    ...
    // Older records have no panzer history and keep the default values.
    ss.str("");
    ss.clear();
    ss << data["panzermaps"];
    ss >> panzerMaps;
...
User::encode( ostream& out, int recnum )
{
    ...
    // Do not add empty panzer state to the usual record.
    if (panzerMaps || panzerHeld) {
        out << '\n' << "panzerMaps = " << panzerMaps;
        out << '\n' << "panzerHeld = " << (panzerHeld ? 1 : 0);
    }
...
User::operator=( const User& ref )
{
    ...
    panzerMaps = ref.panzerMaps;
    panzerHeld = ref.panzerHeld;
