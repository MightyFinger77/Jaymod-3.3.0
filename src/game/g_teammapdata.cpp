...
// Leave room for the final entity and command wrapper.
#define ENTNFO_BUF_SIZE  1200
#define ENTNFO_MSG_SIZE  1400
...
===================
*/
// Format directly into the caller's buffer to avoid per-entity stringstreams.
int G_FormatMapEntity( char* out, int outSize, const mapEntityData_t* mEnt ) {
	char pos[48];

	if( level.ccLayers ) {
		Com_sprintf( pos, sizeof(pos), "%d %d %d",
			int(mEnt->org[0] / 128), int(mEnt->org[1] / 128), int(mEnt->org[2] / 128) );
	} else {
		Com_sprintf( pos, sizeof(pos), "%d %d",
			int(mEnt->org[0] / 128), int(mEnt->org[1] / 128) );
	}
	...
	switch( mEnt->type ) {
		...
		case ME_TANK_DEAD:
		case ME_LANDMINE:
			Com_sprintf( out, outSize, " %d %s %d", mEnt->type, pos, mEnt->data );
			break;

		case ME_PLAYER:
		case ME_PLAYER_REVIVE:
		case ME_PLAYER_DISGUISED:
			Com_sprintf( out, outSize, " %d %x %d",
				mEnt->type, unsigned((mEnt->yaw + 180) / 24), mEnt->data );
			break;

		default:
			Com_sprintf( out, outSize, " %d %s %x %d",
				mEnt->type, pos, unsigned((mEnt->yaw + 180) / 24), mEnt->data );
			break;
	}
...
void G_UpdateTeamMapData_Player(gentity_t* ent, qboolean forceAllied, qboolean forceAxis) {
	...
	if(forceAxis && ent->client && !(ent->client->ps.pm_flags & PMF_LIMBO) /*ent->health > 0*/ ) {
		...
		mEnt->yaw = int( ent->client->ps.viewangles[YAW] );
		mEnt->data = num;
		mEnt->startTime = level.time;
...
// Build the command directly from the accumulated payload.
void G_SendInfo(gentity_t* e, char* buffer, int& bufLen, int& ax_cnt, int& al_cnt, bool& started) {
    char msg[ENTNFO_MSG_SIZE];

    Com_sprintf( msg, sizeof(msg), "%s %d %d\"%s\"",
        started ? "entnfo1" : "entnfo0", ax_cnt, al_cnt, buffer );

    // Send the command
if (!(cvars::g_test.ivalue & G_TEST_SKIP_EINFO))
    trap_SendServerCommand( e-g_entities, msg );
...
static void G_CleanMapEntityTeam( gentity_t* e, mapEntityData_Team_t* teamList )
{
...
void G_SendSpectatorMapEntityInfo( gentity_t* e ) {
    ...
    G_CleanMapEntityTeam(e, teamList);

    for( mEnt = teamList->activeMapEntityData.next; mEnt && mEnt != &teamList->activeMapEntityData; mEnt = mEnt->next ) {

        if( mEnt->type == ME_PLAYER || mEnt->type == ME_PLAYER_DISGUISED || mEnt->type == ME_PLAYER_REVIVE ) {
            /*
            if (g_disableExtendedSpecData.integer)
                continue;
            */
        } else if( mEnt->type != ME_CONSTRUCT && mEnt->type != ME_DESTRUCT && mEnt->type != ME_TANK && mEnt->type != ME_TANK_DEAD && mEnt->type != ME_DESTRUCT_2 )
			continue;

		if( mEnt->singleClient >= 0 && e->s.clientNum != mEnt->singleClient )
			continue;

        entLen = G_FormatMapEntity( entBuf, sizeof(entBuf), mEnt );
        if (bufLen + entLen > 1000) {
            // Send buffer
            G_SendInfo(e, buffer, bufLen, ax_cnt, al_cnt, started);
        }
        memcpy( buffer + bufLen, entBuf, entLen + 1 );
        bufLen += entLen;
        ax_cnt++;
	}
...
//			mapEntityData[1][i].valid = qfalse;
	...
	for( i = 0, ent = g_entities; i < level.num_entities; i++, ent++ ) {
		...
		// Jaybird - g_weapons mine spotting
		if( ent->client->sess.playerType == PC_COVERTOPS || ((cvars::bg_skills.ivalue & SBS_COPS) && ent->client->sess.skill[SK_BATTLE_SENSE] >= 4)) {
			if( ent->health > 0 ) {
				f1 = ent->client->sess.sessionTeam == TEAM_ALLIES ? qtrue : qfalse;
				f2 = ent->client->sess.sessionTeam == TEAM_AXIS ?	qtrue : qfalse;
				...
				if(ent->client->ps.eFlags & EF_ZOOMING) {
					...
					for(j = 0, ent2 = g_entities; j < level.num_entities; j++, ent2++) {
						...
						// Do not give this to spectators
						...
						switch(ent2->s.eType) {
						case ET_MISSILE:
							if( ent2->methodOfDeath == MOD_LANDMINE || ent2->methodOfDeath == MOD_POISON_GAS) {
									...
									if (G_VisibleFromBinoculars( ent, ent2, ent2->r.currentOrigin ))
									{
										...
										case TEAM_AXIS:
											if( !ent2->s.modelindex2 ) {
												...
												if (ent2->landmineSkillWhenPlanted >= 5)
													ent->client->landmineSpotted->count2 += 25;
												else
...
//													int k;
													...
													// Jaybird - Make sure this doesn't fuck with mine names...
													ent->client->landmineSpotted->s.frame = rand() % 20;
											// Keep spotted mines in engineer defuse traces.
													ent->client->landmineSpotted->r.contents = CONTENTS_TRANSLUCENT | CONTENTS_CORPSE;
													trap_LinkEntity( ent->client->landmineSpotted );

											// Coalesce announcements when several mines are found at once.
													if( !ent->client->lastMineSpotAnnounceTime ||
														level.time - ent->client->lastMineSpotAnnounceTime >= MINE_SPOT_ANNOUNCE_DEBOUNCE ) {
														...
														ent->client->lastMineSpotAnnounceTime = level.time;
													}
...
/*													for( k = 0; k < MAX_CLIENTS; k++ ) {
											...
											// Apply the combined score after the scan.
													minesSpotted++;
