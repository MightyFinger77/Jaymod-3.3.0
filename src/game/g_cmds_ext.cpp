...
void G_ready_cmd(gentity_t *ent, unsigned int dwCommand, qboolean state)
{
	...
	if(ent->client->pers.ready == state) CP(va("print \"You are already%s ready!\n\"", status[state]));
	else {
...
// Jaybird - be sure to add here when adding a new ws type!!!
...
int QDECL SortStats( const void *a, const void *b )
{
	...
	if (ca->sess.aWeaponStats[iWeap].subshots)
		shots = ca->sess.aWeaponStats[iWeap].subshots;
	else
		shots = ca->sess.aWeaponStats[iWeap].atts;

	accA = (float)(ca->sess.aWeaponStats[iWeap].hits * 100.0f) / (float)shots;

	if (cb->sess.aWeaponStats[iWeap].subshots)
		shots = cb->sess.aWeaponStats[iWeap].subshots;
	else
		shots = cb->sess.aWeaponStats[iWeap].atts;

	accB = (float)(cb->sess.aWeaponStats[iWeap].hits * 100.0f) / (float)shots;
...
void G_weaponStatsLeaders_cmd(gentity_t* ent, qboolean doTop, qboolean doWindow)
{
	...
	for(iWeap=WS_KNIFE; iWeap<WS_MAX; iWeap++) {
        ...
        // Just bail out if there is not min shots
        if (cQualifyingShots[iWeap] == 0)
            continue;
		...
		for(i=0; i<level.numConnectedClients; i++) {
			...
			if (cl->sess.aWeaponStats[iWeap].subshots)
				shots = cl->sess.aWeaponStats[iWeap].subshots;
			else
		...
		for(i=0; i<cClients; i++) {
			...
			if(((doTop)?acc:(float)wBestAcc+0.999) >= ((doTop)?wBestAcc:acc)) {
				Q_strcat(z, sizeof(z), va(" %d %d %d %d %d %d %d", iWeap+1, aClients[i],
						...
						cl->sess.aWeaponStats[iWeap].subshots,
