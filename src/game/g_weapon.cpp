...
void Weapon_Artillery(gentity_t *ent) {
	...
	if( !G_AvailableAirstrikes( ent ) ) {
		// Limit repeated denial messages while the charge bar remains ready.
		if( level.time - ent->client->lastFireSupportDenyTime >= FIRE_SUPPORT_DENY_DEBOUNCE ) {
			ent->client->lastFireSupportDenyTime = level.time;
    ...
    bool       noEvents)
{
    ...
    // Draw the traced shot path when cheats are enabled and entities are available.
    if (cvars::g_debugBullets.ivalue && g_cheats.integer &&
        level.num_entities < ENTITYNUM_MAX_NORMAL - 64) {
        gentity_t& line = *G_TempEntity( start, EV_DEBUG_BULLET );
        VectorCopy( trx.data.endpos, line.s.origin2 );

        // Red marks player hits; yellow marks world hits.
        VectorSet( line.s.angles2, 1.0f, trx.resultIsPlayer() ? 0.0f : 1.0f, 0.0f );
    }
...
void CalcMuzzlePoint ( gentity_t *ent, int weapon, vec3_t forward, vec3_t right, vec3_t up, vec3_t muzzlePoint ) {
	VectorCopy( ent->r.currentOrigin, muzzlePoint );
	// Start bullet traces at the player's actual eye height.
	muzzlePoint[2] += ent->client->ps.viewheight;
...
void CalcMuzzlePointForActivate ( gentity_t *ent, vec3_t forward, vec3_t right, vec3_t up, vec3_t muzzlePoint ) {
	...
	// Start interaction traces at the player's actual eye height.
	muzzlePoint[2] += ent->client->ps.viewheight;
