...
list<gentity_t*>            __entitiesFree;
list<gentity_t*>::size_type __entitiesFreeSize = 0;
 ...
 *
 * We now use a linked-list and avoid re-using an entity as long as possible
 * since we add free entities to back of list, and pull from front.
 */
gentity_t*
G_Spawn()
{
    stats::entitySpawn.sample( 1 );

    gentity_t* e;

    if (__entitiesFreeSize) {
         e = __entitiesFree.front();
         __entitiesFree.pop_front();
         __entitiesFreeSize--;
         G_InitGentity( e );
         return e;
    }
    ...
    e = &g_entities[ level.num_entities++ ];
    G_InitGentity( e );
...
/*
 * Free an entity and place it on the free list.
 */
void
G_FreeEntity( gentity_t* e )
{
    stats::entityFree.sample( 1 );

	Bot_Event_EntityDeleted(e);
...
================
*/
void G_SetOrigin( gentity_t *ent, vec3_t origin, bool snap ) {
    ...
    if (snap && cvars::g_snap.ivalue & G_SNAP_ITEMS_POS) {
...
==============
*/
void G_SetAngle( gentity_t *ent, const vec3_t angle, bool snap ) {
    ...
    if (snap && cvars::g_snap.ivalue & G_SNAP_ITEMS_APOS) {
        SnapVector(ent->s.apos.trBase);
        SnapVector(ent->s.apos.trDelta);
    }
}
									...
									// deal with any entities in the solid
									{
										...
										for( e = 0; e < listedEntities; e++ ) {
											...
											if( check->client || check->s.eType == ET_CORPSE ) {
												...
												if ( check->s.eType == ET_MISSILE && (check->methodOfDeath == MOD_LANDMINE || check->methodOfDeath == MOD_POISON_GAS)) {
...
void G_ParseCampaigns( void ) {
	...
	if( g_campaignFile.string[0] ) {
		// Jaybird - find the campaign file they specified.
		strcpy(filename, "scripts/");
		strcat(filename, g_campaignFile.string);
	...
	if( !mapFound ) {
		...
		numdirs = trap_FS_GetFileList( "scripts", ".campaign", bigTextBuffer, sizeof( bigTextBuffer ));
		dirptr  = bigTextBuffer;
...
team_t G_GetTeamFromEntity( gentity_t *ent ) {
	switch( ent->s.eType ) {
		...
		case ET_MOVER:		if( !Q_stricmp( ent->classname, "script_mover" ) ) {
							...
							break;
		case ET_CONSTRUCTIBLE:	return (team_t)ent->s.teamNum;
								break;
