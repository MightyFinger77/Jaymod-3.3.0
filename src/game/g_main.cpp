...
vmCvar_t		g_xpMax;
vmCvar_t		g_xpCap;
vmCvar_t		g_ammoRechargeTime;
vmCvar_t		g_healthRechargeTime;
...
vmCvar_t		g_teamDamageMinHits;
vmCvar_t		g_requireClientVersion;
vmCvar_t		g_dbAutoSave;
vmCvar_t		g_playerCount;
vmCvar_t		vote_voteBased;
...
vmCvar_t		g_loseSpreeLevels;

// Custom levels
vmCvar_t		g_levels_battlesense;
vmCvar_t		g_levels_covertops;
vmCvar_t		g_levels_engineer;
vmCvar_t		g_levels_fieldops;
vmCvar_t		g_levels_lightweapons;
vmCvar_t		g_levels_medic;
vmCvar_t		g_levels_soldier;
vmCvar_t		g_defaultSkills;
...
cvarTable_t		gameCvarTable[] = {
	...
	{ &g_privateMessages,	"g_privateMessages",	"0",		CVAR_ARCHIVE },
	{ &g_playDead,			"g_playDead",			"0",		CVAR_ARCHIVE },
	...
	{ &g_xpCap,				"g_xpCap",				"0",		CVAR_ARCHIVE },
	{ &g_ammoRechargeTime,	"g_ammoRechargeTime",	"60000",	CVAR_ARCHIVE },
	{ &g_healthRechargeTime,"g_healthRechargeTime",	"10000",	CVAR_ARCHIVE },
	{ &g_packDistance,		"g_packDistance",		"1",		CVAR_ARCHIVE },
	{ &g_dropHealth,		"g_dropHealth",			"0",		CVAR_ARCHIVE },
	{ &g_dropAmmo,			"g_dropAmmo",			"0",		CVAR_ARCHIVE },
	...
	{ &g_mapConfigs,		"g_mapConfigs",			"",			CVAR_ARCHIVE },
	{ &g_slashKill,			"g_slashKill",			"0",		CVAR_ARCHIVE },
	{ &g_fear,				"g_fear",				"0",		CVAR_ARCHIVE },
	{ &g_reflectFriendlyFire,"g_reflectFriendlyFire","100",		CVAR_ARCHIVE },
	{ &g_teamDamageRestriction,"g_teamDamageRestriction","0",	0 },
	{ &g_teamDamageMinHits,	"g_teamDamageMinHits",	"6",		0 },
	{ &g_requireClientVersion,"g_requireClientVersion","0",		CVAR_ARCHIVE },
	{ &g_dbAutoSave,		"g_dbAutoSave",			"300",		CVAR_ARCHIVE },
	{ &g_playerCount,		"g_playerCount",		"0",		CVAR_ROM },
	{ &vote_voteBased,		"vote_voteBased",		"0",		0 },
	{ &vote_minPercent,		"vote_minPercent",		"0",		0 },
	...
	{ &g_headshot,			"g_headshot",			"0",		CVAR_ARCHIVE },
	{ &g_shortcuts,			"g_shortcuts",			"0",		CVAR_ARCHIVE },
	{ &g_campaignFile,		"g_campaignFile",		"",			0 },
	{ &g_truePing,			"g_truePing",			"1",		0 },
	{ &sv_fps,				"sv_fps",				"20",		CVAR_SYSTEMINFO | CVAR_ARCHIVE },
	{ &g_shoutcastpassword,	"g_shoutcastpassword",	"",			0 },
	...
	{ &g_soldiers,			"g_soldiers",			"0",		CVAR_ARCHIVE },
	...
	{ &g_levels_covertops,	"g_levels_covertops",	"",			CVAR_ARCHIVE },
	{ &g_levels_engineer,	"g_levels_engineer",	"",			CVAR_ARCHIVE },
	{ &g_levels_fieldops,	"g_levels_fieldops",	"",			CVAR_ARCHIVE },
	{ &g_levels_lightweapons,"g_levels_lightweapons","",		CVAR_ARCHIVE },
	{ &g_levels_medic,		"g_levels_medic",		"",			CVAR_ARCHIVE },
	{ &g_levels_soldier,	"g_levels_soldier",		"",			CVAR_ARCHIVE },
	{ &g_defaultSkills,		"g_defaultSkills",		"",			CVAR_ARCHIVE },
...
// jpw
	...
	{ &g_maxlivesRespawnPenalty, "g_maxlivesRespawnPenalty", "0", CVAR_ARCHIVE|CVAR_LATCH, 0, qtrue },
	...
	{ &vote_allow_generic,		"vote_allow_generic", "1", 0, 0, qfalse, qfalse },
	...
	{ &vote_allow_matchrestart,	"vote_allow_matchrestart", "1", 0, 0, qfalse, qfalse },
...
#ifdef FEATURE_LUA
	{ &lua_modules, "lua_modules", "", CVAR_ARCHIVE | CVAR_NORESTART, 0, qfalse },
	{ &lua_allowedModules, "lua_allowedModules", "", CVAR_ARCHIVE | CVAR_NORESTART, 0, qfalse },
	{ &lua_reportWebhook, "lua_reportWebhook", "", CVAR_ARCHIVE | CVAR_NORESTART, 0, qfalse },
#endif
};
...
vmMain( intptr_t command, intptr_t arg0, intptr_t arg1, intptr_t arg2, intptr_t arg3, intptr_t arg4, intptr_t arg5, intptr_t arg6 ) {
	...
	case GAME_CLIENT_CONNECT:
		{
			static string msg;
			if (ClientConnect( msg, (int)arg0, (qboolean)arg1, (qboolean)arg2 ))
				return (intptr_t)msg.c_str();
			else
				return 0;
		}
...
void G_CheckForCursorHints( gentity_t *ent ) {
	...
	// Jaybird - adjust for custom hitboxes
	...
	if( tr->entityNum == ENTITYNUM_WORLD ) {
		...
		if ( traceEnt->client && traceEnt->client->sess.sessionTeam == ent->client->sess.sessionTeam) {
			if(ps->stats[ STAT_PLAYER_CLASS ] == PC_MEDIC && 
				...
				!( traceEnt->client->ps.pm_flags & PMF_LIMBO)) {
					hintDist	= CH_REVIVE_DIST; // JPW NERVE matches weapon_syringe in g_weapon.c 
		...
		// Jaybird - live uniform stealing
		if(( cvars::bg_covertops.ivalue & COPS_LIVEUNI) && traceEnt->client && ent->client->sess.playerType == PC_COVERTOPS && !G_HasJayFlag( traceEnt, 1, JF_LOSTPANTS ) && traceEnt->client->sess.sessionTeam != ent->client->sess.sessionTeam ) {
			...
			if( DotProduct( eforward, pforward ) > 0.6f ) {
				...
				// Scale elapsed milliseconds to the cursor hint range.
				hintVal = traceEnt->stealProgress * 255 / STEAL_UNIFORM_TIME;
				if( hintVal > 255 ) {
					hintVal = 255;
				}
			}
		...
		if(checkEnt) {
			...
			switch (checkEnt->s.eType) {
				case ET_CORPSE:
					...
					if(( cvars::bg_covertops.ivalue & COPS_LIVEUNI ) && G_HasJayFlag( ent, 1, JF_LOSTPANTS ) && BODY_TEAM(traceEnt) == ent->client->sess.sessionTeam ) {
						// Jaybird - live uni stealing
						hintDist	= 48;
						hintType	= HINT_UNIFORM;
						hintVal		= BODY_VALUE(traceEnt);
						if( hintVal > 255 ) {
					...
					else if( g_classChange.integer && BODY_TEAM(traceEnt) == ent->client->sess.sessionTeam ) {
						// Jaybird - class stealing
						hintDist	= 48;
						hintType	= HINT_UNIFORM;
						hintVal		= BODY_VALUE(traceEnt);
						if( hintVal > 255 ) {
				...
				case ET_ITEM: 
				{
					...
					switch(it->giType) {
						...
						case IT_WEAPON: {
								...
								if( !canPickup ) {
									canPickup = G_CanPickupWeapon( (weapon_t)it->giTag, ent );
								}
		...
		if ( cv->vmCvar ) {
			...
			if(cv->modificationCount != cv->vmCvar->modificationCount) {
				...
				if( cv->vmCvar == &g_soldierChargeTime ) {
					level.soldierChargeTime[0] = int( g_soldierChargeTime.value * level.soldierChargeTimeModifier[0] );
					level.soldierChargeTime[1] = int( g_soldierChargeTime.value * level.soldierChargeTimeModifier[1] );
					chargetimechanged = qtrue;
				} else if( cv->vmCvar == &g_medicChargeTime ) {
					level.medicChargeTime[0] = int( g_medicChargeTime.value * level.medicChargeTimeModifier[0] );
					level.medicChargeTime[1] = int( g_medicChargeTime.value * level.medicChargeTimeModifier[1] );
					chargetimechanged = qtrue;
				} else if( cv->vmCvar == &g_engineerChargeTime ) {
					level.engineerChargeTime[0] = int( g_engineerChargeTime.value * level.engineerChargeTimeModifier[0] );
					level.engineerChargeTime[1] = int( g_engineerChargeTime.value * level.engineerChargeTimeModifier[1] );
					chargetimechanged = qtrue;
				} else if( cv->vmCvar == &g_LTChargeTime ) {
					level.lieutenantChargeTime[0] = int( g_LTChargeTime.value * level.lieutenantChargeTimeModifier[0] );
					level.lieutenantChargeTime[1] = int( g_LTChargeTime.value * level.lieutenantChargeTimeModifier[1] );
					chargetimechanged = qtrue;
				}
				else if( cv->vmCvar == &g_covertopsChargeTime ) {
					level.covertopsChargeTime[0] = int( g_covertopsChargeTime.value * level.covertopsChargeTimeModifier[0] );
					level.covertopsChargeTime[1] = int( g_covertopsChargeTime.value * level.covertopsChargeTimeModifier[1] );
					chargetimechanged = qtrue;
				}
...
#ifdef FEATURE_LUA
	if ( G_LuaCvarsChanged() ) {
		G_LuaRestart();
		G_LuaHook_InitGame( level.time, rand(), 1 );
	}
#endif
}
...
void G_InitGame( int levelTime, int randomSeed, int restart ) {
	...
	*/
	...
	level.warmupModificationCount = cvars::g_warmup.modificationCount;
	...
	{
		const char *proto = Info_ValueForKey( cs, "protocol" );
		const char *gname = Info_ValueForKey( cs, "gamename" );
		const int slen = (int)strlen( cs );
		G_Printf( "serverinfo %d/%d protocol=%s gamename=%s\n",
			slen, MAX_INFO_STRING, proto[0] ? proto : "(missing)", gname[0] ? gname : "(missing)" );
		if ( !proto[0] || atoi( proto ) != 84 )
			G_Printf( S_COLOR_RED "WARNING: getinfo protocol must be 84 for ET 2.60 / ETL clients. This value is owned by etlded, not qagame.\n" );
	}
	...
	// Jaybird - print OSP style servertime
	{
		time_t t;
		struct tm *lt;
		char s[38];
		...
		if(time(&t)) {
			lt = localtime(&t);
			strftime(s, sizeof(s), "%Y%m%d%H%M%S %H:%M:%S (%d %b %Y)", lt);
			G_LogPrintf("ServerTime: %s\n", s);
		}
	}

	// Jaybird - map-specific configs
    ...
    // Jaybird - set up map object
    string err;
    currentMap = &mapDB.fetchByKey( level.rawmapname, err, true );
    if (cvars::gameState.ivalue != GS_WARMUP) {
        currentMap->timestamp = time(NULL);
        currentMap->count++;
    }
...
//	G_SpawnScriptCamera();
	...
	// Optional check for the cached weapon lookup tables.
	if ( cvars::g_test.ivalue & G_TEST_VERIFY_TABLES ) {
		BG_VerifyWeaponItemTables();
	}
...
void G_ShutdownGame( int restart ) {
	...
	G_WriteSessionData( (qboolean)restart );

    for (int i = 0; i < MAX_CLIENTS; i++) {
		g_clientObjects[i].shutdown();
    }

    for (int i = 0; i < MAX_GENTITIES; i++) {
        g_entityObjects[i].shutdown();
    }

	// zinx - realistic hitboxes
	mdx_cleanup();

	// Advance persistent panzer state after a completed map, but not a restart.
	if (!restart)
		userDB.panzerTickAll( cvars::g_panzerMapLimit.ivalue );
...
int QDECL SortRanks( const void *a, const void *b ) {
	...
	if( g_gametype.integer == GT_WOLF_LMS ) {
		...
		for( totalXP[0] = totalXP[1] = 0, i = 0; i < SK_NUM_SKILLS; i++ ) {
			totalXP[0] += int( ca->sess.skillpoints[i] );
			totalXP[1] += int( cb->sess.skillpoints[i] );
		}
				...
				for( j = 0; j < level.numConnectedClients; j++ ) {
					...
					if ( i == SK_BATTLE_SENSE || i == SK_LIGHT_WEAPONS ) {
						if( cl->sess.skillpoints[i] > highestskillpoints ) {
							highestskillpoints = int( cl->sess.skillpoints[i] );
							highestskillpointsclient = j;
						}
					} else {
						if( (cl->sess.skillpoints[i] - cl->sess.startskillpoints[i]) > highestskillpointsincrease ) {
							highestskillpointsincrease = int( cl->sess.skillpoints[i] - cl->sess.startskillpoints[i] );
							highestskillpointsclient = j;
						}
					} // b017
				}
...
void CheckExitRules( void ) {
	...
	if ( g_timelimit.value && !level.warmupEndTime ) {
		// OSP
...
void CheckWolfMP() {
    ...
    if(g_gametype.integer >= GT_WOLF) {
        if(cvars::gameState.ivalue == GS_PLAYING || cvars::gameState.ivalue == GS_INTERMISSION) {
            if(level.intermissiontime && cvars::gameState.ivalue != GS_INTERMISSION)
                cvars::gameState.set( GS_INTERMISSION );
            return;
        }
        ...
        if(cvars::gameState.ivalue == GS_WARMUP) {
            ...
            {
                const int delay = (cvars::g_warmup.ivalue < 1) ? 0 : cvars::g_warmup.ivalue;

                level.warmupEndTime = level.time + (delay * 1000);
                cvars::gameState.set( GS_WARMUP_COUNTDOWN );
                trap_SetConfigstring( CS_WARMUP, va( "%i", level.warmupEndTime ));
            }
        }

        // if the warmup time has counted down, restart
        if(cvars::gameState.ivalue == GS_WARMUP_COUNTDOWN) {
            if(level.time > level.warmupEndTime) {
                level.warmupEndTime += 10000;
...
void CheckVote( void ) {
	...
	if(level.time - level.voteInfo.voteTime >= VOTE_TIME) {
		if( vote_voteBased.integer ) {
			...
			if( voted >= vote_minPercent.integer * total / 100 && level.voteInfo.voteYes > pcnt * voted / 100 ) {
				// Do the action
				AP("cpm \"^5Vote passed!\n\"");
...
void G_TagLinkEntity( gentity_t* ent, int msec ) {
	...
	if (!(parent->s.eFlags & EF_PATH_LINK)) {
		if( parent->s.pos.trType == TR_LINEAR_PATH ) {
			...
			pos = int(ent->backdelta * (MAX_SPLINE_SEGMENTS));
...
void G_RunFrame( int levelTime ) {
    ...
    if (process.pendingReload) {
        // clear
        process.setPendingReload( false );
    ...
    if (process.pendingShutdown) {
        // clear
        process.setPendingShutdown( false );
	...
	// Periodically save the Admin DB so an unclean shutdown loses less data.
	if( g_dbAutoSave.integer > 0 ) {
		if( level.nextDbSaveTime == 0 ) {
			level.nextDbSaveTime = level.frameTime + g_dbAutoSave.integer * 1000;
		} else if( level.frameTime >= level.nextDbSaveTime ) {
			levelDB.save();
			userDB.save();
			mapDB.save();
			level.nextDbSaveTime = level.frameTime + g_dbAutoSave.integer * 1000;
	...
	// Publish the active player count for map scripts, updating only on change.
	{
		int playerCount = TeamCount( -1, TEAM_AXIS ) + TeamCount( -1, TEAM_ALLIES );
		if( playerCount != g_playerCount.integer ) {
			trap_Cvar_Set( "g_playerCount", va( "%i", playerCount ) );
			trap_Cvar_Update( &g_playerCount );
		}
	}
	...
	for (i = 0; i < level.numConnectedClients; i++)
        g_clientObjects[ level.sortedClients[i] ].run();
	...
	if(level.gameManager) {
		int axisMines, alliedMines;
		G_CountTeamLandminesBoth( &axisMines, &alliedMines );
		level.gameManager->s.otherEntityNum = MAX_TEAM_LANDMINES - axisMines;
		level.gameManager->s.otherEntityNum2 = MAX_TEAM_LANDMINES - alliedMines;
	}
