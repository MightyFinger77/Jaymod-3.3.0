...
qboolean G_PushPlayer(gentity_t *ent, gentity_t *victim) 
{
    ...
    // Use server health because the networked dead flag may be a frame behind.
    if (ent->health <= 0)
        return qfalse;

    if (victim->health <= 0 || (victim->client->ps.eFlags & (EF_PLAYDEAD | EF_DEAD)))
        return qfalse;
...
void G_UpdateJaymodCS() {
    ...
    Info_SetValueForKey( cs, "y", cvars::bg_aimSpreadTurnScale.svalue );
...
qboolean G_LiveUniformSteal(gentity_t *ent, gentity_t *traceEnt) {
	...
	if (ent->client->ps.powerups[PW_BLUEFLAG] || ent->client->ps.powerups[PW_REDFLAG])
		return qfalse;
	if (traceEnt->stealProgress < STEAL_UNIFORM_TIME ) {
		// Track uniform-steal progress in elapsed milliseconds.
		traceEnt->stealProgress += level.time - level.previousTime;
		if( traceEnt->stealProgress > STEAL_UNIFORM_TIME )
			traceEnt->stealProgress = STEAL_UNIFORM_TIME;
...
void G_AuthCheck( gentity_t *ent, char *version ) {
	...
	if( *version ) {
		...
		// Optionally require clients to match the server version.
		if( g_requireClientVersion.integer ) {
			ostringstream msg;
			msg << "Version mismatch: client " << (ent - g_entities)
				<< " has \"" << version << "\", server requires \""
				<< JAYMOD_title << "\"\n";
			G_LogPrintf( msg.str().c_str() );
			trap_DropClient( ent - g_entities,
				"This server requires the " JAYMOD_title " client.",
				cache::kickTime );
			return;
		}
	}
