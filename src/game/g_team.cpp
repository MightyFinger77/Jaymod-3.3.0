...
if (cvars::g_test.ivalue & G_TEST_SKIP_TINFO)
    return;
	...
	for (i = 0, cnt = 0; i < level.numConnectedClients; i++) {
		...
		if (player->inuse && player->client->sess.sessionTeam == team) {
			...
			Com_sprintf( entry, sizeof(entry), " %i %i %i %i %i %i", level.sortedClients[i], player->client->pers.teamState.location[0] / 64, player->client->pers.teamState.location[1] / 64, player->client->pers.teamState.location[2] / 64, h, player->s.powerups );
...
{
	teamInfo[team_num].team_lock = (match_latejoin.integer == 0 && cvars::gameState.ivalue == GS_PLAYING) ? qtrue : qfalse;
...
{
    if ((g_doWarmup.integer || (g_gametype.integer == GT_WOLF_LMS && g_lms_lockTeams.integer) ||
         level.warmupEndTime > (level.time + 10*1000) ) && cvars::gameState.ivalue == GS_WARMUP && G_checkReady())
    {
...
{
    gamestate_t gs = (gamestate_t)cvars::gameState.ivalue;
	...
	if((team_num == TEAM_AXIS || team_num == TEAM_ALLIES)) {
		...
		if( g_gametype.integer != GT_WOLF_LMS ) {
			...
			if(team_maxplayers.integer > 0 && team_maxplayers.integer <= cnt) {
				G_printFull(va("The %s team is full!", aTeams[team_num]), ent);
				return(qfalse);
			} else if( cvars::gameState.ivalue == GS_PLAYING && g_lms_lockTeams.integer && (!(ent->client->pers.invite & team_num))) {
				G_printFull(va("The %s team is LOCKED!", aTeams[team_num]), ent);
				return(qfalse);
			}
		}
