...
qboolean G_ScriptAction_StartAnimation( gentity_t* ent, char *params ) {
	...
	ent->s.weapon = int( 1000.0f / atoi(token) );
...
qboolean G_ScriptAction_SetChargeTimeFactor( gentity_t* ent, char *params ) {
	...
	if( !Q_stricmp( playerclass, "soldier" ) ) {
		...
		level.soldierChargeTime[team] = int( g_soldierChargeTime.value * factor );
	} else if( !Q_stricmp( playerclass, "medic" ) ) {
		level.medicChargeTimeModifier[team] = factor;
		level.medicChargeTime[team] = int( g_medicChargeTime.value * factor );
	} else if( !Q_stricmp( playerclass, "engineer" ) ) {
		level.engineerChargeTimeModifier[team] = factor;
		level.engineerChargeTime[team] = int( g_engineerChargeTime.value * factor );
	} else if( !Q_stricmp( playerclass, "fieldops" ) ) {
		level.lieutenantChargeTimeModifier[team] = factor;
		level.lieutenantChargeTime[team] = int( g_LTChargeTime.value * factor );
		...
		level.covertopsChargeTime[team] = int( g_covertopsChargeTime.value * factor );
	}
...
qboolean G_ScriptAction_DisableMessage( gentity_t *ent, char *params ) {
	...
	while ((target = G_FindByTargetname( target, token ))) {
		target->s.aiState = (aistateEnum_t)1;
	}
...
qboolean G_ScriptAction_GotoMarker( gentity_t *ent, char *params )
{
	...
	if (!params || ent->scriptStatus.scriptStackChangeTime < level.time) {			// we are waiting for it to reach destination
		...
		if (ent->s.eType == ET_MOVER) {
			...
			ent->s.pos.trDuration = int( 1000.0f * (dist / speed) );
...
qboolean G_ScriptAction_VoiceAnnounce( gentity_t *ent, char *params) {
	...
	sysmsg = (sysMsg_t)G_GetSysMessageNumber( token );
	if(sysmsg == -1) {
...
qboolean G_ScriptAction_SetState( gentity_t *ent, char *params ) {
	...
	if( !found ) {
		target = &g_entities[MAX_CLIENTS-1];
		while( ++target < &g_entities[level.num_entities] ) {
			if( !target->inuse || !target->scriptName ) {
				continue;
			}
			if( Q_stricmp( target->scriptName, name ) ) {
				continue;
			}
...
qboolean G_ScriptAction_SetBotGoalState( gentity_t *ent, char *params ) {
	...
	Q_strncpyz( name, token, sizeof(name) );
	if(!*name) {
		G_Error( "G_Scripting: setbotgoalstate must have a name and an state\n" );
	}


	while ((token = COM_ParseExt( &pString, qfalse )) && token && token[0]) {
		if( !Q_stricmp( token, "axis" ) ) {
			...
			teamFlags[0] = qtrue;
			teamFlags[1] = qtrue;
		} else if( !Q_stricmp( token, "inactive" ) ) {
			entState = qtrue;
		} else if( !Q_stricmp( token, "active" ) ) {
			entState = qfalse;
	} else {
			G_Error( "G_Scripting: setbotgoalstate with invalid state '%s'\n", token );
		}
	}
	...
	while ((target = G_FindByTargetnameFast( target, name, hash ))) {
		...
		if (teamFlags[1]) {
			if (entState) {
				target->aiInactive |= (1<<TEAM_ALLIES);
			} else {
				target->aiInactive &= ~(1<<TEAM_ALLIES);
			}
		}
...
qboolean G_ScriptAction_SetAASState( gentity_t *ent, char *params ) {
	...
	token = COM_ParseExt( &pString, qfalse );
	if(!token || !*token) {
		G_Error( "G_Scripting: setaasstate must have a name and a state [enabled|disabled|avoid]\n" );
	}

	Q_strncpyz( targetname, token, sizeof(targetname) );
	...
	if(!Q_stricmp( token, "enabled" )) {
		flags = AAS_AREA_ENABLED;
	} else if(!Q_stricmp( token, "disabled" )) {
		flags = AAS_AREA_DISABLED;
	} else if(!Q_stricmp( token, "avoid" )) {
		flags = AAS_AREA_AVOID;
	} else {
		G_Error( "G_Scripting: setaasstate: invalid state: %s\n", token );
		return qtrue;
	}
...
qboolean G_ScriptAction_SetBotGoalPriority( gentity_t *ent, char *params ) {
	...
	Q_strncpyz( name, token, sizeof(name) );
	if( !*name ) {
		G_Error( "G_Scripting: setbotgoalpriority must have a name and a level\n" );
	}
	...
	while ((token = COM_ParseExt( &pString, qfalse )) && token && token[0]) {
		if( !Q_stricmp( token, "axis" ) ) {
			...
			token = COM_ParseExt( &pString, qfalse );
			if( !token || !*token ) {
				G_Error( "G_Scripting: setbotgoalpriority has 'level' with no value\n" );
			}
			priority = atoi(token);
		} else {
			G_Error( "G_Scripting: setbotgoalpriority with invalid token '%s'\n", token );
		}
	}
	...
	while ((target = G_FindByTargetnameFast( target, name, hash ))) {
		...
		if (teamFlags[1]) {
			target->goalPriority[1] = priority;
		}
	}
...
/*
===================     
etpro_ScriptAction_CreateValues

Jaybird - imitate etpro behavior
Creates a new entity from a script
...
// rain - bleah.  I don't like the way this works, but it's good enough
// for now.
#define MAX_DELETE_MATCHES 32
#define ELIMINATE(field, val, func)						\
	for (x = 0; x < numMatches; x++) {					\
		if (func(matches[x]->field, val))				\
			matches[x] = NULL;							\
	}
...
qboolean etpro_ScriptAction_DeleteEntity( gentity_t *ent, char *params )
{
	...
	char		*p;
	char		key[MAX_TOKEN_CHARS], value[MAX_TOKEN_CHARS];
	gentity_t	*matches[MAX_DELETE_MATCHES];
	int hash;
	int x, numMatches = 0;
	vec3_t vec;
	...
	for (x = 0; x < MAX_DELETE_MATCHES; x++)
		matches[x] = NULL;

	if (g_scriptDebug.value)
	...
	while( 1 )
	{
		...
		if (!numMatches) {
			// first pass - we don't have any matches, so search the entire
			// entity list
			...
			if (!Q_stricmp(key, "scriptname")) {
				...
				while ((matches[numMatches] = e = G_FindByTargetnameFast(e, value, hash)) != NULL)
					numMatches++;
			} else if (!Q_stricmp(key, "origin")) {
				for (x = 0; x < level.num_entities; x++) {
					if (VectorCompare(g_entities[x].s.origin, vec)) {
						matches[numMatches] = &g_entities[x];
						numMatches++;
					}
				}
			} else
				G_Printf("^3Warning: Can't match for \"%s\" while deleting.\n", key);
		} else {
			// second pass - eliminating entities that don't match from
			// our match list

			
			if (!Q_stricmp(key, "scriptname")) {
				ELIMINATE(scriptName, value, Q_stricmp)
			} else if (!Q_stricmp(key, "classname")) {
				ELIMINATE(classname, value, Q_stricmp)
			} else if (!Q_stricmp(key, "targetname")) {
				ELIMINATE(targetname, value, Q_stricmp)
			} else if (!Q_stricmp(key, "origin")) {
				ELIMINATE(s.origin, vec, !VectorCompare)
			} else
				G_Printf("^3Warning: Can't match for \"%s\" while deleting.\n", key);
		}
	}

	for (e = NULL, x = 0; x < numMatches; x++) {
		if (e != NULL && matches[x] != NULL) {
			G_Printf("^1Error: Too many matches while deleting entity\n");
			return qfalse;
		}
		e = matches[x];
	}
	...
	if (e == NULL) {
		G_Printf("^1Error: No matches while deleting entity\n");
		return qfalse;
	}

	if (g_scriptDebug.integer)
		G_Printf("%d : (%s) Freeing entity #%d (classname \"%s\", scriptname \"%s\", targetname \"%s\")\n",
			level.time, ent->scriptName, e->s.number, e->classname, e->scriptName, e->targetname);
