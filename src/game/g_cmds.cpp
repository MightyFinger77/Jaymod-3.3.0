...
void Cmd_ListBotGoals_f( gentity_t* ent ) {
	...
	for( t = TEAM_AXIS; t <= TEAM_ALLIES; t = (team_t)( t+1 ) ) {
...
void Cmd_Give_f (gentity_t *ent)
{
    ...
    string s_subcmd;
    SEngine::argv( 1, s_subcmd );
    const bool all = (str::toLower( s_subcmd ) == "all");

    string s_number;
    SEngine::argv( 2, s_number );
    const int i_number = atoi( s_number.c_str() );

    if (s_subcmd == "xp") {
        const int amount = i_number ? i_number : 100;
            for (int i = 0; i < SK_NUM_SKILLS; i++) {
                const skillType_t sk = (skillType_t)i;
                G_AddSkillPoints( ent, sk, amount );
                G_DebugAddSkillPoints( ent, sk, amount, "give skill" );
            }
        return;
    }
    ...
    if (s_subcmd == "skill") {
        if (i_number) {
            // bump specific skill
            if (i_number >= 0 && i_number < SK_NUM_SKILLS) {
                const skillType_t sk = (skillType_t)i_number;
                int amount = 20;
                if (ent->client->sess.skill[sk] < NUM_SKILL_LEVELS-1)
                    amount = int( skillLevels[sk][ent->client->sess.skill[sk] + 1] - ent->client->sess.skillpoints[sk] );
    ...
    if (s_subcmd == "allammo") {
        for (int i = 1 ; i < WP_NUM_WEAPONS; i++)
            Add_Ammo( ent, i, i_number, qtrue );
    }

    if (all || s_subcmd == "keys") {
...
void Cmd_God_f (gentity_t *ent)
{
    ...
    const char* const name = ConcatArgs( 1 );
    ...
    const char* const state = (ent->flags & FL_GODMODE) ? "ON" : "OFF";

    trap_SendServerCommand( ent-g_entities, va("print \"godmode %s\"", state ));
}
...
void Cmd_Kill_f( gentity_t *ent )
{
	...
	// Jaybird - slashkill
	if (g_slashKill.integer == KILL_DISABLE)
		return;

	// Jaybird - do not let poisoned people /kill
	if (G_IsPoisoned(ent))
		return;

	if(ent->client->sess.sessionTeam == TEAM_SPECTATOR ||  (ent->client->ps.pm_flags & PMF_LIMBO) || level.match_pause != PAUSE_NONE) {
		return;
	}
	...
	// Jaybird - slashKill
	ent->client->pers.slashKillTime = ent->client->ps.classWeaponTime;

	attacker = g_entities + ent->client->lasthurt_client;

	// Jaybird - FEAR!
	// Test everything under the sun...
	if(	attacker && attacker->client &&
		g_fear.integer && ( level.time - ent->client->lasthurt_time ) < g_fear.integer &&
		attacker->client->sess.sessionTeam != ent->client->sess.sessionTeam &&
		attacker->health > 0 ) {
		player_die( ent, attacker, attacker, ent->health + 150, MOD_FEAR );
	} else {
		player_die(ent, ent, ent, (cvars::gameState.ivalue == GS_PLAYING) ? 100000 : 135, MOD_SUICIDE);
	}
	...
	if( !Q_stricmp( teamstr, "follow1" ) ) {
		...
		*team = PickTeam( clientNum );
		if(!G_teamJoinCheck( *team, &g_entities[clientNum] )) {
			*team = (team_t)((TEAM_AXIS | TEAM_ALLIES) & ~*team);
		}
	}
}
...
qboolean SetTeam( gentity_t* ent, const char* teamName, qboolean force, weapon_t w1, weapon_t w2, qboolean setweapons ) {
	...
	if( team != TEAM_SPECTATOR ) {
		...
		if(g_noTeamSwitching.integer && (team != ent->client->sess.sessionTeam && ent->client->sess.sessionTeam != TEAM_SPECTATOR ) && cvars::gameState.ivalue == GS_PLAYING && !force ) {
	...
	// The active-player limit only blocks spectators joining the game.
	if ( team != TEAM_SPECTATOR && !force &&
	     g_maxGameClients.integer > 0 &&
	     level.numNonSpectatorClients >= g_maxGameClients.integer &&
	     client->sess.sessionTeam == TEAM_SPECTATOR ) {
		CP("cp \"The game is full.\n\"");
		return qfalse;
	}
	...
	// Move limbo followers when their target changes teams.
	if( team != oldTeam ) {
		int j;
		for( j = 0; j < level.numConnectedClients; j++ ) {
			gentity_t* follower = &g_entities[ level.sortedClients[j] ];

			if( follower->client->sess.spectatorClient != clientNum )
				continue;
			if( !( follower->client->ps.pm_flags & PMF_LIMBO ) )
				continue;
			if( follower->client->sess.sessionTeam == team )
				continue;
	...
	if(team != oldTeam) {
		// Jaybird - don't delete stats.  This isn't too fair...
...
//				G_AddClientToFireteam( ent-g_entities, ft->joinOrder[0] );
	...
	// Mark the time they joined the team
	client->pers.joinedTeamTime = level.time;
...
void StopFollowing( gentity_t *ent ) {
	...
	if( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
		...
		// Jaybird - Get rid of spectator jumping out of a disoriented player bug
		angle[ROLL] = 0;
...
int G_PlayerCount( gentity_t* ent, int playerType ) {
	int i, j, cnt;

	if( playerType == -1 ) { // we aint checking for a class, so always include ourselves
		cnt = 1;
...
// Note - we do not check soldiers because they are the default fallback class
bool G_IsClassDisabled( gentity_t* ent, int playerType, bool quiet )
{
    int count, pcount;
    char* msg1 = "^7Sorry, all available^1";
    char* msg2 = " ^7positions are taken.";

    if (ent->client->sess.sessionTeam == TEAM_SPECTATOR)
        return true;


    count = G_PlayerCount( ent, -1 );
    pcount = G_PlayerCount( ent, playerType );

    switch (playerType) {
	case PC_ENGINEER:
		if (cvars::bg_maxEngineers.ivalue == -1)
			return false;

        if (BG_IsPercent(cvars::bg_maxEngineers.svalue)) {
            int percent = cvars::bg_maxEngineers.ivalue;

            if (pcount >= ceil(count * percent * 0.01f))
                return true;
        } else {
		    if (pcount >= cvars::bg_maxEngineers.ivalue) {
			    if (!quiet)
				    CP(va("cp \"%s %s%s\" 1", msg1, "SOLDIER", msg2));
			    return true;
		    }
        }
		break;
	case PC_MEDIC:
		if (cvars::bg_maxMedics.ivalue == -1)
			return false;

        if (BG_IsPercent(cvars::bg_maxMedics.svalue)) {
            int percent = cvars::bg_maxMedics.ivalue;

            if (pcount >= ceil(count * percent * 0.01f))
                return true;
        } else {
		    if (pcount >= cvars::bg_maxMedics.ivalue) {
			    if (!quiet)
				    CP(va("cp \"%s %s%s\" 1", msg1, "MEDIC", msg2));
			    return true;
		    }
        }
		break;
	case PC_FIELDOPS:
		if (cvars::bg_maxFieldOps.ivalue == -1)
			return false;

        if (BG_IsPercent(cvars::bg_maxFieldOps.svalue)) {
            int percent = cvars::bg_maxFieldOps.ivalue;

            if (pcount >= ceil(count * percent * 0.01f))
                return true;
        } else {
		    if (pcount >= cvars::bg_maxFieldOps.ivalue) {
			    if (!quiet)
				    CP(va("cp \"%s %s%s\" 1", msg1, "FIELD OPS", msg2));
			    return true;
		    }
        }
		break;
	case PC_COVERTOPS:
		if (cvars::bg_maxCovertOps.ivalue == -1)
			return false;

        if (BG_IsPercent(cvars::bg_maxCovertOps.svalue)) {
            int percent = cvars::bg_maxCovertOps.ivalue;

            if (pcount >= ceil(count * percent * 0.01f))
                return true;
        } else {
		    if (pcount >= cvars::bg_maxCovertOps.ivalue) {
			    if (!quiet)
				    CP(va("cp \"%s %s%s\" 1", msg1, "COVERT OPS", msg2));
			    return true;
		    }
        }
		break;
...
qboolean G_IsWeaponDisabled( gentity_t* ent, weapon_t weapon, qboolean quiet ) {
	...
	switch(weapon) {
	case WP_PANZERFAUST:
		// Enforce the map lockout even when the normal team limit is disabled.
		if (cvars::g_panzerMapLimit.ivalue > 0) {
			User& user = *connectedUsers[ ent->s.number ];

			if (!user.isNull() && !user.fakeguid && user.panzerMaps < 0) {
				if (!quiet) {
					CP( va( "cp \"^7You had the ^1PANZERFAUST ^7for %d maps.\n"
					        "^7You may use it again in ^3%d ^7map%s.\" 2",
					        cvars::g_panzerMapLimit.ivalue,
					        -user.panzerMaps,
					        (user.panzerMaps == -1) ? "" : "s" ));
				}
				return qtrue;
		...
		if (cvars::bg_maxPanzers.ivalue == -1 || cvars::bg_panzerWar.ivalue)
			return qfalse;

        if (BG_IsPercent(cvars::bg_maxPanzers.svalue)) {
            int percent = cvars::bg_maxPanzers.ivalue;

            if (wcount >= ceil(count * percent * 0.01f))
                return qtrue;
        } else {
		    if (wcount >= cvars::bg_maxPanzers.ivalue) {
			    if (!quiet)
				    CP(va("cp \"%s %s%s\" 1", msg1, "PANZERFAUST", msg2));
			    return qtrue;
            }
        }
		break;
	case WP_MOBILE_MG42:
		if (cvars::bg_maxMG42s.ivalue == -1)
			return qfalse;

        if (BG_IsPercent(cvars::bg_maxMG42s.svalue)) {
            int percent = cvars::bg_maxMG42s.ivalue;

            if (wcount >= ceil(count * percent * 0.01f))
                return qtrue;
        } else {
		    if (wcount >= cvars::bg_maxMG42s.ivalue) {
			    if (!quiet)
				    CP(va("cp \"%s %s%s\" 1", msg1, "MG42", msg2));
			    return qtrue;
		    }
        }
		break;
	case WP_FLAMETHROWER:
		if (cvars::bg_maxFlamers.ivalue == -1)
			return qfalse;

        if (BG_IsPercent(cvars::bg_maxFlamers.svalue)) {
            int percent = cvars::bg_maxFlamers.ivalue;

            if (wcount >= ceil(count * percent * 0.01f))
                return qtrue;
        } else {
		    if (wcount >= cvars::bg_maxFlamers.ivalue) {
			    if (!quiet)
				    CP(va("cp \"%s %s%s\" 1", msg1, "FLAME THROWER", msg2));
			    return qtrue;
		    }
        }
		break;
	case WP_MORTAR:
		if (cvars::bg_maxMortars.ivalue == -1)
			return qfalse;

        if (BG_IsPercent(cvars::bg_maxMortars.svalue)) {
            int percent = cvars::bg_maxMortars.ivalue;

            if (wcount >= ceil(count * percent * 0.01f))
                return qtrue;
        } else {
		    if (wcount >= cvars::bg_maxMortars.ivalue) {
			    if (!quiet)
				    CP(va("cp \"%s %s%s\" 1", msg1, "MORTAR", msg2));
			    return qtrue;
		    }
        }
		break;
	case WP_KAR98:
	case WP_CARBINE:
		if (cvars::bg_maxGrenLaunchers.ivalue == -1)
			return qfalse;

        if (BG_IsPercent(cvars::bg_maxGrenLaunchers.svalue)) {
            int percent = cvars::bg_maxGrenLaunchers.ivalue;

            if (wcount >= ceil(count * percent * 0.01f))
                return qtrue;
        } else {
		    if (wcount >= cvars::bg_maxGrenLaunchers.ivalue) {
			    if (!quiet)
				    CP(va("cp \"%s %s%s\" 1", msg1, "GRENADE LAUNCHER", msg2));
			    return qtrue;
		    }
        }
		break;
	case WP_M97:
		...
		if (cvars::bg_maxM97s.ivalue == -1)
			return qfalse;

        if (BG_IsPercent(cvars::bg_maxM97s.svalue)) {
            int percent = cvars::bg_maxM97s.ivalue;

            if (wcount >= ceil(count * percent * 0.01f))
                return qtrue;
        } else {
		    if (wcount >= cvars::bg_maxM97s.ivalue) {
			    if (!quiet)
				    CP(va("cp \"%s %s%s\" 1", msg1, "M97", msg2));
			    return qtrue;
		    }
        }
		break;
...
void Cmd_Team_f( gentity_t *ent, unsigned int dwCommand, qboolean fValue ) {
	...
	w =		(weapon_t)atoi( weap );
	w2 =	(weapon_t)atoi( weap2 );
...
void Cmd_Follow_f( gentity_t *ent, unsigned int dwCommand, qboolean fValue ) {
	...
	if ( ent->client->sess.sessionTeam != TEAM_SPECTATOR ) {
		SetTeam( ent, "spectator", qfalse, (weapon_t)-1, (weapon_t)-1, qfalse );
	}
...
void Cmd_FollowCycle_f( gentity_t *ent, int dir ) {
	...
	// Jaybird - check if we're spec'ing the other team (very bad!)
	// This should fix the spec bug where you spec the other team, but since I can't
	// reproduce this, nor can anyone tell me how to, it is hard to test.  In the end,
	// it doesn't hurt to have this extra check.
    if (ent->client->sess.spectatorState == SPECTATOR_FOLLOW && ent->client->sess.sessionTeam != level.clients[original].sess.sessionTeam)
        StopFollowing(ent);
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	text = chatText;
    if (text.length() >= MAX_SAY_TEXT) {
        text.resize(MAX_SAY_TEXT - 1);
    }

    // Do censor checks
	if (g_censor.integer && !cmd::entityHasPermission( ent, priv::base::censorImmunity )) {
        if (censorDB.filter(text)) {
			G_CensorPenalize(ent);
		}
	}

	// Next, replace chat shortcuts
	G_ChatShortcuts(ent, text);

	switch (mode) {
	default:
	case SAY_ALL:
		G_LogPrintf("say: %s: %s\n", ent->client->pers.netname, text.c_str());
        name = string(ent->client->pers.netname) + Q_COLOR_ESCAPE + COLOR_WHITE + ": ";
		color = COLOR_GREEN;
		break;
	case SAY_BUDDY:
		localize = true;
        ...
        name = "[lof](" + string(ent->client->pers.netname) + Q_COLOR_ESCAPE + COLOR_WHITE + ") (" + loc + "): ";
		color = COLOR_YELLOW;
		break;
	case SAY_TEAM:
		localize = true;
...
// NERVE - SMF
void G_VoiceTo( gentity_t *ent, gentity_t *other, int mode, const char *id, qboolean voiceonly, const string msg ) {
	int color;
	char *cmd;
	...
	// Apply the text-chat ignore list to voice chats too.
	if ( ent != other &&
	     COM_BitCheck( other->client->sess.ignoreClients, ent - g_entities ) ) {
		return;
	}
...
void G_Voice( gentity_t *ent, gentity_t *target, int mode, const char *id, qboolean voiceonly, string msg ) {
	...
	// Jaybird - No censor check
	if ( ent->voiceChatSquelch >= 30000 && !cmd::entityHasPermission( ent, priv::base::censorImmunity )) {
    ...
    // Do censor checks
	if (g_censor.integer && !cmd::entityHasPermission( ent, priv::base::censorImmunity )) {
        if (censorDB.filter(msg)) {
			G_CensorPenalize(ent);
		}
	}

	// Next, replace chat shortcuts
	G_ChatShortcuts(ent, msg);

	if ( target ) {
		G_VoiceTo( ent, target, mode, id, voiceonly, msg );
		return;
	}

	// echo the text to the console
	if ( g_dedicated.integer ) {
		G_Printf( "voice: %s %s %s\n", ent->client->pers.netname, id, msg.c_str());
	}
	...
	if( mode == SAY_BUDDY ) {
		...
		for( j = 0; j < level.numConnectedClients; j++ ) {
			...
			G_VoiceTo(ent, &g_entities[level.sortedClients[j]], mode, id, voiceonly, msg);
		}
	} else {
...
static void Cmd_Voice_f( gentity_t *ent, int mode, qboolean arg0, qboolean voiceonly ) {
	...
	if( mode != SAY_BUDDY ) {
		...
		trap_Argv( (arg0) ? 0 : 1, cmd, sizeof( cmd ) );
		msg = ConcatArgs( (arg0) ? 1 : 2 );
		G_Voice(ent, NULL, mode, cmd, voiceonly, msg);
	} else {
		char buffer[16];
		int index;
		...
		trap_Argv( (arg0) ? 2 + index : 3 + index, cmd, sizeof( cmd ) );
		msg = ConcatArgs( (arg0) ? 3 + index : 4 + index );
		G_Voice(ent, NULL, mode, cmd, voiceonly, msg);
	}
}
...
qboolean Cmd_CallVote_f( gentity_t *ent, unsigned int dwCommand, qboolean fRefCommand ) {
	...
	for (int i = 0; i < 3; i++) {
		if(strchr( arg1, badChars[i] ) || strchr( arg2, badChars[i] ) ) {
...
void Cmd_Vote_f( gentity_t *ent ) {
	...
	if ( ent->client->pers.complaintEndTime > level.time && cvars::gameState.ivalue == GS_PLAYING && g_complaintlimit.integer ) {
		...
		if ( msg[0] == 'y' || msg[1] == 'Y' || msg[1] == '1' ) {
			...
			if( !cl->pers.localClient ) {
				...
				if( num <= 0 || !G_FindFreeComplainIP( cl, &ip ) ) {
					trap_DropClient( cl - level.clients, "kicked after too many complaints.", cl->sess.referee ? 0 : cache::kickTime );
			...
			G_LoseKillSkillPoints( other, (meansOfDeath_t)ent->sound2to1, (hitRegion_t)ent->sound1to2, ent->sound2to3 ? qtrue : qfalse );
		} else {
...
qboolean Do_Activate_f(gentity_t *ent, gentity_t *traceEnt) {
	...
	if(( ent->client->pers.cmd.buttons & BUTTON_WALKING) || ( ent->client->ps.eFlags & EF_CROUCHING ) || ( ent->client->ps.eFlags & EF_PRONE ))
		walking = qtrue;

	// Rate-limit mounting without affecting dismounts.
	if( ( traceEnt->s.eType == ET_MOVER && G_TankIsMountable( traceEnt, ent ) ) ||
	    G_EmplacedGunIsMountable( traceEnt, ent ) ) {
		if( level.time - ent->client->mountRateWindow >= MOUNT_RATE_WINDOW ) {
			ent->client->mountRateWindow = level.time;
			ent->client->mountRateCount = 0;
		}

		if( ent->client->mountRateCount >= MOUNT_RATE_MAX &&
		    level.time - ent->client->lastMountTime < MOUNT_RATE_MIN_INTERVAL ) {
			return qfalse;
		}

		ent->client->mountRateCount++;
		ent->client->lastMountTime = level.time;
	}
...
void Cmd_Activate2_f( gentity_t *ent ) {
	...
	if (tr.entityNum >= 0) {
		...
		if (traceEnt->client) {
			...
			if( DotProduct( eforward, pforward ) > 0.6f && ent->client->sess.sessionTeam != traceEnt->client->sess.sessionTeam )
				G_LiveUniformSteal(ent, traceEnt);
			else if (traceEnt->client->ps.eFlags & EF_PLAYDEAD)
				G_DragCorpse(ent, traceEnt);
			else
	...
	if (tr.entityNum >= 0) {
		...
		if (traceEnt->s.eType == ET_CORPSE && BODY_TEAM(traceEnt)) {
			if(BODY_TEAM(traceEnt) == ent->client->sess.sessionTeam ) {
				if (G_HasJayFlag( ent, 1, JF_LOSTPANTS ) && (cvars::bg_covertops.ivalue & COPS_LIVEUNI))
					G_UniformRestore(ent, traceEnt);
				else
...
void Cmd_WeaponStat_f ( gentity_t* ent ) {
    ...
    extWeaponStats_t stat = (extWeaponStats_t)atoi(buffer);
...
void G_CalcClientAccuracies( void ) {
	...
	for( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		if( g_entities[i].inuse ) {
			for( j = 0; j < WS_MAX; j++ ) {
				if (level.clients[i].sess.aWeaponStats[j].subshots)
					shots += level.clients[i].sess.aWeaponStats[j].subshots;
				else
...
void Cmd_Ignore_f (gentity_t* ent ) {
	...
	if(!*cmd) {
		trap_SendServerCommand( ent-g_entities, "print \"usage: ignore <clientname|slot#>.\n\"\n" );
		return;
	}
...
void Cmd_UnIgnore_f (gentity_t* ent ) {
	...
	if(!*cmd) {
		trap_SendServerCommand( ent-g_entities, "print \"usage: unignore <clientname|slot#>.\n\"\n" );
		return;
	}
...
void ClientCommand( int clientNum ) {
	...
	// Jaybird - silent commands
    if( cmd[0] == '!') {
        if (cmd::entityHasPermission( ent, priv::base::commandSilent )) {
    		cmd::process( &g_clientObjects[clientNum], true );
        } else {
		    trap_SendServerCommand( clientNum, "print \"You are not authorized to use silent commands.\n\"" );
        }
        return;
	}
...
void G_CensorPenalize(gentity_t *ent)
{
	...
	if (g_censorPenalty.integer & CNSRPNLTY_TMPMUTE) {
        G_MutePlayer(ent, "censor");

        CPx(ent-g_entities, "chat \"^1You've been auto-muted for language.\"");
	}
    ...
    // Kill the offender, but don't gib them
...
void Cmd_AuthCheck_f( gentity_t *ent, unsigned int dwCommand, qboolean fValue ) {
	char version[64];

	trap_Argv( 1, version, sizeof( version ));
	G_AuthCheck( ent, version );
}
