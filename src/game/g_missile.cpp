...
void G_MissileImpact( gentity_t *ent, trace_t *trace, int impactDamage ) {
	...
	// Update the damage based on speed.
	if( ent->s.weapon == WP_KNIFE ) {
		ent->damage = int( VectorLength( ent->s.pos.trDelta ) / 20 );
		if( ent->damage > 75 )
			ent->damage = 75;
	}
	...
	if(other->classname && Q_stricmp(other->classname, "func_explosive") == 0) {
		CPx( other->r.ownerNum, "cpm \"Getting into func_explosive.\n\"" );
	...
	if ( ( !other->takedamage || !ent->damage ) && ( ent->s.eFlags & ( EF_BOUNCE | EF_BOUNCE_HALF ) ) ) {
		...
		// Stop knives that become trapped and keep bouncing.
		if( ent->s.weapon == WP_KNIFE ) {
			ent->count2++;
			if( ent->count2 >= KNIFE_MAX_BOUNCES ||
			    VectorLengthSquared( ent->s.pos.trDelta ) < SQR( 60 ) ) {
				G_SetOrigin( ent, trace->endpos );
				return;
			}
		}
...
/*	if (other->takedamage && ent->s.density == 1)
	...
	if (other->takedamage || other->dmgparent) {
		if ( ent->damage ) {
			...
			// Tons of things to check here...
			if (
				( cvars::bg_weapons.ivalue & SBW_PSTHKNIVES ) &&
				other->client &&
...
void G_ExplodeMissile( gentity_t *ent ) {
	...
	if( ent->splashDamage ) {
		...
		if( ( ent->s.weapon == WP_DYNAMITE && ( ent->etpro_misc_1 & 1 ) ) || ent->s.weapon == WP_SATCHEL ) {
			etpro_RadiusDamage( origin, ent, ent->parent, ent->splashDamage, ent->splashRadius, ent, ent->splashMethodOfDeath, RADIUS_SCOPE_CLIENTS );
			G_TempTraceIgnorePlayersAndBodies();
			etpro_RadiusDamage( origin, ent, ent->parent, ent->splashDamage, ent->splashRadius, ent, ent->splashMethodOfDeath, RADIUS_SCOPE_NOCLIENTS );
...
void G_MissileDie( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod ) {
	...
	// Jaybird - vulnerable satchel
	if( self->s.weapon == WP_SATCHEL ) {
		gentity_t *owner = g_entities + self->s.clientNum;

		if( owner->client ) {
			owner->client->ps.ammo[WP_SATCHEL_DET] = 0;
			owner->client->ps.ammoclip[WP_SATCHEL_DET] = 0;
			owner->client->ps.ammoclip[WP_SATCHEL] = 1;
			G_AddEvent( owner, EV_NOAMMO, 0 );
		}
	}
}
...
void Landmine_Check_Ground (gentity_t *self)
{
	...
	if (self->clipmask & CONTENTS_BODY) {
...
//		G_SetOrigin( ent, tr.endpos );
	...
	} else if (ent->s.weapon == WP_KNIFE && !VectorLength(ent->s.pos.trDelta)) {
		// Knife is resting, spawn an item out of it
		gitem_t *newKnifeItem = BG_FindItemForWeapon(WP_KNIFE);
		vec3_t	velocity = { 0, 0, 0 };
		gentity_t *newKnifeEnt;

		// Give it a little space to spawn
		ent->r.currentOrigin[2] += 4;

		newKnifeEnt  = LaunchItem( newKnifeItem, ent->r.currentOrigin, velocity, ent->r.ownerNum );

		// Set Angles
		newKnifeEnt->s.apos.trBase[PITCH] = 75;
		newKnifeEnt->s.apos.trBase[YAW] = rand() % 360;
		newKnifeEnt->touch = ThrowingKnifeTouch;

		newKnifeEnt->think = MagicSink;
		newKnifeEnt->nextthink = level.time + 30000;
...
void G_RunFlamechunk( gentity_t *ent, int msec ) {
	...
	// Use elapsed time so flame range does not depend on sv_fps.
	float	frametime = (float)msec / 1000.f;
	...
	if ( level.time - ent->timestamp > 50 ) {
		speed -= frametime * FLAME_FRICTION_PER_SEC;
	...
	// Move the chunk
	VectorScale( ent->s.pos.trDelta, frametime, add );
...
/* Disabled this code section. If we revamp flamer, we'll add appropriate CVAR/debugging.
 * --Mike
 */
#if 0
	// Show debugging bbox
...
/*
==============
G_CountTeamLandminesBoth

Counts both teams' landmines in one pass.
==============
*/
void G_CountTeamLandminesBoth ( int* axisCount, int* alliedCount ) {
	...
	for( i = MAX_CLIENTS ; i<level.num_entities ; i++, e++ ) {
		...
		switch (e->methodOfDeath) {
			case MOD_LANDMINE:
				break;

			case MOD_POISON_GAS:
				if (e->poisonGasWeaponType != WP_LANDMINE_PGAS)
					continue;
				break;
		...
		if ( e->s.teamNum % 4 == TEAM_AXIS ) {
			axis++;
		} else if ( e->s.teamNum % 4 == TEAM_ALLIES ) {
			allied++;
		}
	}
...
bool G_SweepForLandmines( gentity_t* ent, float radius, int team ) {
    ...
    // check again no sooner than 500ms from now
    ent->client->pers.sweepForLandminesAlarm = level.timeCurrent + 500;
	...
	for (int i = MAX_CLIENTS; i < level.num_entities; i++, e++) {
		...
		if( e->s.teamNum % 4 != team && e->s.teamNum < 4) {
			VectorSubtract( ent->r.currentOrigin, e->r.currentOrigin, dist );
...
void LandMinePostTrigger(gentity_t* self) {
	switch (self->s.weapon) {
		case WP_LANDMINE_BBETTY:
			...
			self->think = G_BouncingBettyTrip;
			break;

		case WP_LANDMINE_PGAS:
			self->nextthink = level.time;
			self->think = G_PoisonGasTrip;
			break;
...
void G_LandmineThink( gentity_t *self ) {
	...
	for( i = 0; i < cnt; i++) {
		...
		// cs: by default, bots will not trigger mines that they can see
...
qboolean G_LandmineSnapshotCallback( int entityNum, int clientNum ) {
	...
	// Jaybird - Shoutcaster capability
	// Do not let people following a SC see mines
	if( clEnt->client->sess.sessionTeam == TEAM_SPECTATOR && clEnt->client->sess.shoutcaster && clEnt->client->sess.spectatorState != SPECTATOR_FOLLOW ) {
		return qtrue;
	}
...
gentity_t *fire_grenade (gentity_t *self, vec3_t start, vec3_t dir, int grenadeWPID) {
	...
	switch (grenadeWPID) {
		...
		case WP_KNIFE: // Jaybird - throwing knives
			noExplode = qtrue;
			break;
	}
...
// jpw
	...
	switch(grenadeWPID) {
		case WP_KNIFE:
			bolt->classname				= "throwing_knife";
			bolt->s.eFlags				= EF_BOUNCE_HALF | EF_BOUNCE;
			bolt->nextthink				= level.time + 30000;
			bolt->think					= MagicSink;
			bolt->methodOfDeath			= MOD_THROWING_KNIFE;
			bolt->r.contents			= CONTENTS_MISSILECLIP;

			// Give it some bounds so item doesn't spawn outside of world bounds
			VectorSet( bolt->r.mins, -ITEM_RADIUS, -ITEM_RADIUS, 0 );
			VectorSet( bolt->r.maxs, ITEM_RADIUS, ITEM_RADIUS, ITEM_RADIUS*2 );
			break;
		case WP_GPG40:
    ...
    case WP_MORTAR_SET:
      ...
      break;
		...
		case WP_LANDMINE_BBETTY:
			bolt->accuracy				= 0;
			bolt->s.teamNum				= self->client->sess.sessionTeam + 4;
			bolt->classname				= "landmine_bbetty";
			bolt->damage				= 0;
			bolt->splashRadius			= 300;
			bolt->methodOfDeath			= MOD_LANDMINE;
			bolt->splashMethodOfDeath	= MOD_LANDMINE;
...
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	if (cvars::bg_panzerWar.ivalue) {
		bolt->damage = int( bolt->damage * 0.33f );
		bolt->splashDamage = int( bolt->splashDamage * 0.33f );
		bolt->splashRadius = int( bolt->splashRadius * 0.66f );
	}
...
//	bolt->clipmask = MASK_SHOT;
	...
	bolt->s.pos.trType = ( g_soldiers.integer & SDR_PANZERGRAV ) ? TR_GRAVITY_LOW : TR_LINEAR;
	bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;		// move a bit on the very first frame
	VectorCopy( start, bolt->s.pos.trBase );
// JPW NERVE
