...
void Fire_Lead_Ext( gentity_t *ent, gentity_t *actor, float spread, int damage, vec3_t muzzle, vec3_t forward, vec3_t right, vec3_t up, int mod )
{
	int seed = rand() & 255;
    ...
    if (actor->client)
        g_clientObjects[actor->s.number].bulletModel->adjustStartPoint( muzzle );

    // Do the trace
    TraceContext trx( *ent, *actor, MASK_SHOT, reinterpret_cast<vec_t(&)[3]>(*muzzle), end );
    if (!AbstractBulletModel::fireWorld( trx ))
        return;

    // Didn't really hit anything, we're done
    if (trx.data.surfaceFlags & SURF_NOIMPACT) {
        gentity_t& fx = *G_TempEntity( trx.data.endpos, EV_MG42BULLET_HIT_WALL );
        fx.s.otherEntityNum  = ent->s.number;
        fx.s.otherEntityNum2 = actor->s.number;
        fx.s.effect1Time     = seed;
        return;
    }

    gentity_t& traceEnt = g_entities[ trx.data.entityNum ];
    SnapVectorTowards( trx.data.endpos, muzzle );

    // Send bullet impact
    if (traceEnt.takedamage && traceEnt.client) {
        gentity_t& fx = *G_TempEntity( trx.data.endpos, EV_MG42BULLET_HIT_FLESH );
        fx.s.eventParm       = traceEnt.s.number;
        fx.s.otherEntityNum  = ent->s.number;
        fx.s.otherEntityNum2 = actor->s.number;
        fx.s.effect1Time     = seed;
    } else {
        // Ridah, bullet impact should reflect off surface
        float dot = DotProduct( forward, trx.data.plane.normal );

        vec3_t reflect;
        VectorMA( forward, -2*dot, trx.data.plane.normal, reflect );
        ...
        // If the trace hit water, send that info to client to render the effect
        if (trx.water) {
            vec3_t v;
            VectorSubtract( trx.data.endpos, trx.start, v );
    ...
    if (trx.resultIsPlayer())
        g_clientObjects[traceEnt.s.number].takeBulletDamageFrom( trx, g_clientObjects[actor->s.number], damage, 0, mod );
    else if (traceEnt.takedamage)
        G_Damage( &traceEnt, ent, actor, forward, trx.data.endpos, damage, 0, mod );
}
...
// interpolation
void clamp_playerbehindgun (gentity_t *self, gentity_t *other, const vec3_t dang) {
...
void mg42_fire( gentity_t *other ) {
	...
	// Align the emplaced gun muzzle with the player's line of sight.
	{
		vec3_t eye, delta;
		float  depth;

		VectorCopy( other->client->ps.origin, eye );
		eye[2] += other->client->ps.viewheight;

		VectorSubtract( muzzle, eye, delta );
		depth = DotProduct( delta, forward );
		if( depth < 0 )
			depth = 0;

		VectorMA( eye, depth, forward, muzzle );
	}
...
void mg42_think (gentity_t *self) {
	...
	if( self->mg42weapHeat ) {
		self->mg42weapHeat -= int( 300.f * FRAMETIME * 0.001f );
...
static void landmine_setup( gentity_t *ent ) {
	trace_t		tr;
	vec3_t		end;
	...
	ent->s.time			= int( ent->s.angles[1] + 90 );
...
// I'm just putting this here because there are so many references to it
...
int G_GetTargetDynamite( int *list, int listSize, gentity_t* target ) {
	...
	for( dyn = G_FindDynamite( NULL ); dyn; dyn = G_FindDynamite( dyn ) ) {
		...
		for( team = TEAM_AXIS; team <= TEAM_ALLIES; team = (team_t)( team+1 ) ) {
			vec3_t mins, maxs;
