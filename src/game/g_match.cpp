...
void G_loadMatchGame(void)
{
	...
	if(server_autoconfig.integer > 0 && (!(z_serverflags.integer & ZSF_COMP) || level.newSession)) {
		G_configSet(g_gametype.integer, (server_autoconfig.integer == 1) ? qtrue : qfalse);
...
#if 0 // moved to cgame for audio/visual rendering
...
//	G_Printf("mod: %d, Index: %d, dmg: %d\n", mod, G_weapStatIndex_MOD(mod), dmg_ref);
	...
	if(g_gametype.integer >= GT_WOLF &&
	  targ->client->sess.sessionTeam == attacker->client->sess.sessionTeam) {
		...
		// Jaybird - track friendly hits
		ref = G_weapStatIndex_MOD(mod);
		if( dmg > 0 && ref < WS_MAX ) {
			attacker->client->sess.aWeaponStats[ref].friendlyHits++;
			attacker->client->sess.aWeaponStats[ref].totalHits++;
		}
...
{
	for (int i = 0; i < MOD_NUM_MODS; i++) {
		if (iWeaponMOD == aWeapMOD[i].mod)
			return aWeapMOD[i].ws;
	}
	...
	for(i=WS_KNIFE; i<WS_MAX; i++) {
		if(refEnt->client->sess.aWeaponStats[i].atts || refEnt->client->sess.aWeaponStats[i].hits ||
		  refEnt->client->sess.aWeaponStats[i].deaths || refEnt->client->sess.aWeaponStats[i].kills) {
			dwWeaponMask |= (1 << i);
            if (logPrint) {
                if (g_logOptions.integer & LOGOPTS_EXTENDED_WEAPS) {
                    // Extended weapons stats for new hit zones
    			    Q_strcat(strWeapInfo, sizeof(strWeapInfo), va(" %d %d %d %d %d %d %d %d %d %d",
    					    ...
    					    refEnt->client->sess.aWeaponStats[i].bodyshots,
    					    refEnt->client->sess.aWeaponStats[i].leftarmshots,
    					    refEnt->client->sess.aWeaponStats[i].rightarmshots,
    					    refEnt->client->sess.aWeaponStats[i].leftlegshots,
    					    refEnt->client->sess.aWeaponStats[i].rightlegshots));
                } else {
                    // etmain default weapon stats
                ...
                // players get a different set than log does
    			Q_strcat(strWeapInfo, sizeof(strWeapInfo), va(" %d %d %d %d %d %d",
	    				refEnt->client->sess.aWeaponStats[i].hits,
                        refEnt->client->sess.aWeaponStats[i].atts,
			    		refEnt->client->sess.aWeaponStats[i].subshots,
				    	refEnt->client->sess.aWeaponStats[i].kills,
                        refEnt->client->sess.aWeaponStats[i].deaths,
    					refEnt->client->sess.aWeaponStats[i].headshots));
            }
...
void G_deleteStats(int nClient)
{
	...
	// Revives and headshots are per-map award values.
	cl->sess.revives = 0;
	cl->sess.headshots = 0;
...
#define GETVAL(x) if((tmp = strchr(tmp, ' ')) == NULL) return; x = atoi(++tmp);
	...
	// CHRUKER: b015 - These only gets generated when there are some weaponstats. This is what the client expects.
	if (dwWeaponMask != 0) {
		...
		if(dwDumpType == EOM_WEAPONSTATS) {
			...
			if(!(cl->pers.clientFlags & CGF_STATSDUMP)) {
				if((cl->pers.autoaction & AA_STATSALL) || cl->pers.mvCount > 0) {
					...
					else CP(va("ws %s\n", G_createStats(g_entities + pid, false)));
				}
			}
...
void G_statsPrint(gentity_t *ent, int nType)
{
	...
	if(trap_Argc() < 2) {
		...
		CP(va("%s %s\n", cmd, G_createStats(g_entities + pid, false)));
	}
}
