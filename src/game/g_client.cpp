...
qboolean AddWeaponToPlayer( gclient_t *client, weapon_t weapon, int ammo, int ammoclip, qboolean setcurrent ) {
	// Count a panzer map only after the weapon is actually granted.
	if (weapon == WP_PANZERFAUST && client->pers.connected == CON_CONNECTED) {
		User& user = *connectedUsers[ client->ps.clientNum ];
		if (!user.isNull() && !user.fakeguid)
			user.panzerHeld = true;
	}
...
void ClientUserinfoChanged( int clientNum ) {
    ...
    if (!connectedUsers[clientNum]) {
        G_Printf("ClientUserinfoChanged: client %d has no user record\n", clientNum);
        return;
    }
    ...
    if ( *s && !strcmp( s, "localhost" ) ) {
        ...
        // Omni-bot uses ip=localhost. Do not promote a dedicated x64
        // server to listen-host / referee just because a bot connected.
	...
	// Humans that drop cl_mac after presenting one are kicked.
	// Omni-bot never sends cl_mac; do not recurse into ClientDisconnect
	// from ClientUserinfoChanged while AddBot is still in ClientConnect.
	if (!(ent->r.svFlags & SVF_BOT) && !user.mac.empty() && mac.empty()) {
...
ClientConnect( string& outmsg, int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	// IP flood fill check (humans only — Omni-bot shares ip=localhost)
	if (!isBot && !( ent->r.svFlags & SVF_BOT ))
	{
...
#ifdef FEATURE_LUA
	{
		char luaReason[MAX_STRING_CHARS];
		luaReason[0] = '\0';
		if ( G_LuaHook_ClientConnect( clientNum, firstTime, isBot, luaReason, sizeof( luaReason ) ) ) {
			outmsg = luaReason[0] ? luaReason : "Connection rejected by Lua";
			return true;
		}
	}
#endif
...
#ifdef FEATURE_LUA
	G_LuaHook_ClientSpawn( ent - g_entities, revived, qfalse, qtrue );
#endif
}
