...
bool needClientFlagsUpdated = false;
            ...
            intptr_t arg11  )
{
	switch ( cmd ) {
	case CG_INIT:
		CG_Init( (int)arg0, (int)arg1, (int)arg2, (qboolean)arg3);
		cgs.initing = qfalse;
		return 0;
	case CG_SHUTDOWN:
		CG_Shutdown();
	...
	case CG_DRAW_ACTIVE_FRAME:
		CG_DrawActiveFrame( (int)arg0, (stereoFrame_t)arg1, (qboolean)arg2 );
		return 0;
...
vmCvar_t	cg_killspreesounds;
vmCvar_t	cg_pmsounds;
vmCvar_t	cg_obituaryLocation;
vmCvar_t	cg_obituaryFilter;
vmCvar_t	cg_watermarkOpacity;
vmCvar_t	cg_althud;
vmCvar_t	cg_jaymiscflags;
vmCvar_t	cg_pmblock;
vmCvar_t	cg_drawDisconnectIcon;
vmCvar_t	cg_popupTime;
vmCvar_t	cg_popupFadeTime;
vmCvar_t	cg_popupWaitTime;
vmCvar_t	cg_numPopups;
vmCvar_t	cg_drawClock;
vmCvar_t	cg_muzzleFlash;
vmCvar_t    cg_mapZoom;

vmCvar_t    cg_locationMode;
vmCvar_t    cg_locationMaxChars;
vmCvar_t    cg_locationJustify;

vmCvar_t    cg_drawSpeed;
vmCvar_t    cg_speedRefresh;

vmCvar_t    cg_autoRate;
vmCvar_t    cg_savedRate;
...
//	{ &cg_draw2D, "cg_draw2D", "1", CVAR_CHEAT }, // JPW NERVE changed per atvi req to prevent sniper rifle zoom cheats
	...
	{ &cg_cycleBinoculars, "cg_cycleBinoculars", "0", CVAR_ARCHIVE },
	...
	{ &cg_killspreesounds,	"cg_killspreesounds",	"1",	CVAR_ARCHIVE },
	...
	{ &cg_popupWaitTime,	"cg_popupWaitTime",		"5000",	CVAR_ARCHIVE },
	{ &cg_numPopups,		"cg_numPopups",			"10",	CVAR_ARCHIVE },
    ...
    { &cg_locationMode,     "cg_locationMode",      "0",    CVAR_ARCHIVE },
    { &cg_locationMaxChars, "cg_locationMaxChars",  "25",    CVAR_ARCHIVE },
    { &cg_locationJustify,  "cg_locationJustify",   "0",    CVAR_ARCHIVE },

    { &cg_drawSpeed,        "cg_drawSpeed",         "0",    CVAR_ARCHIVE },
    { &cg_speedRefresh,     "cg_speedRefresh",      "100",  CVAR_ARCHIVE },

    { &cg_autoRate,         "cg_autoRate",          "1",    CVAR_ARCHIVE },
    { &cg_savedRate,        "cg_savedRate",         "0",    CVAR_ARCHIVE },
...
================
*/
void CG_SetMACAddress() {
    string mac;
    GetMACAddress( mac );
    cvars::cl_mac.set( mac );
}
...
void CG_UpdateCvars( void ) {
    ...
    qboolean jayFlags = qfalse;

    if (!cvarsLoaded)
        return;

    cvarTable_t* cv = cvarTable;
    for (int i = 0; i < cvarTableSize; i++, cv++) {
        if(cv->vmCvar) {
...
void CG_setClientFlags(void)
{
	...
	trap_Cvar_Set("cg_uinfo", va("%d %d %d",
											...
											cvars::cl_timeNudge.ivalue,
											cvars::cl_maxpackets.ivalue
									   ));
}
...
static void CG_RegisterSounds( void ) {
	...
	cgs.media.goat = trap_S_RegisterSound( "sound/osp/goat.wav", qfalse );
	// OSP
	...
	for(i = 0; i < 3; i++) {
		...
		cgs.media.sfx_brassSound[BRASSSOUND_SHELL][i] =	trap_S_RegisterSound (va("sound/weapons/misc/shell_shotgun%i.wav",	i + 1),	qfalse );
	...
	// Jaybird - hitsounds
	cgs.media.hitsound_general = trap_S_RegisterSound( "sound/hitsounds/general.wav", qfalse );

	cgs.media.hitsound_head  = trap_S_RegisterSound( "sound/hitsounds/head.wav",  qfalse );
	cgs.media.hitsound_hand  = trap_S_RegisterSound( "sound/hitsounds/hand.wav",  qfalse );
	cgs.media.hitsound_torso = trap_S_RegisterSound( "sound/hitsounds/torso.wav", qfalse );
	cgs.media.hitsound_foot  = trap_S_RegisterSound( "sound/hitsounds/foot.wav",  qfalse );

	cgs.media.hitsound_fhead  = trap_S_RegisterSound( "sound/hitsounds/fhead.wav",  qfalse );
	cgs.media.hitsound_fhand  = trap_S_RegisterSound( "sound/hitsounds/fhand.wav",  qfalse );
	cgs.media.hitsound_ftorso = trap_S_RegisterSound( "sound/hitsounds/ftorso.wav", qfalse );
	cgs.media.hitsound_ffoot  = trap_S_RegisterSound( "sound/hitsounds/ffoot.wav",  qfalse );
	...
	cgs.media.killingSpree[1] = trap_S_RegisterSound( "sound/killingspree/killspree2.wav", qfalse );
	cgs.media.killingSpree[2] = trap_S_RegisterSound( "sound/killingspree/killspree3.wav", qfalse );
	cgs.media.killingSpree[3] = trap_S_RegisterSound( "sound/killingspree/killspree4.wav", qfalse );
	cgs.media.killingSpree[4] = trap_S_RegisterSound( "sound/killingspree/killspree5.wav", qfalse );
	cgs.media.killingSpree[5] = trap_S_RegisterSound( "sound/killingspree/killspree6.wav", qfalse );

	cgs.media.losingSpree[0] = trap_S_RegisterSound( "sound/killingspree/losespree1.wav", qfalse );
	cgs.media.losingSpree[1] = trap_S_RegisterSound( "sound/killingspree/losespree2.wav", qfalse );
	cgs.media.losingSpree[2] = trap_S_RegisterSound( "sound/killingspree/losespree3.wav", qfalse );
	cgs.media.losingSpree[3] = trap_S_RegisterSound( "sound/killingspree/losespree4.wav", qfalse );
	cgs.media.losingSpree[4] = trap_S_RegisterSound( "sound/killingspree/losespree5.wav", qfalse );
	cgs.media.losingSpree[5] = trap_S_RegisterSound( "sound/killingspree/losespree6.wav", qfalse );

	cgs.media.multikill[0] = trap_S_RegisterSound( "sound/multikill/doublekill.wav", qfalse );
	cgs.media.multikill[1] = trap_S_RegisterSound( "sound/multikill/triplekill.wav", qfalse );
	cgs.media.multikill[2] = trap_S_RegisterSound( "sound/multikill/multikill.wav", qfalse );
	cgs.media.multikill[3] = trap_S_RegisterSound( "sound/multikill/ultrakill.wav", qfalse );
	cgs.media.multikill[4] = trap_S_RegisterSound( "sound/multikill/monsterkill.wav", qfalse );
...
//	cgs.media.cursor = trap_R_RegisterShaderNoMip( "ui/assets/mvcursor.tga" );
	...
	cgs.media.friendShader			= trap_R_RegisterShaderNoMip( "gfx/2d/friendlycross" );
...
//	cgs.media.commandCentreWoodShader		= trap_R_RegisterShaderNoMip( "ui/assets2/commandMap" );
    ...
    cgs.media.commandCentreMapMask                  = trap_R_RegisterShaderNoMip("gfx/2d/mapmask");
    ...
    cgs.media.polygonCubeCO = trap_R_RegisterModel( "models/polygons/cubeCO.md3" );
    cgs.media.polygonCubeFO = trap_R_RegisterModel( "models/polygons/cubeFO.md3" );
    cgs.media.polygonCubeOO = trap_R_RegisterModel( "models/polygons/cubeOO.md3" );

	cgs.media.machinegunBrassModel  = trap_R_RegisterModel( "models/weapons2/shells/m_shell.md3" );
	cgs.media.panzerfaustBrassModel = trap_R_RegisterModel( "models/weapons2/shells/pf_shell.md3" );
	cgs.media.shotgunBrassModel     = trap_R_RegisterModel( "models/weapons2/shells/shotgun.md3" );
	...
	cgs.media.disconnectShader = trap_R_RegisterShader( "sprites/net" );
	cgs.media.poisonShader = trap_R_RegisterShader( "sprites/poisoned" );
    ...
    cgs.media.mutedShader = trap_R_RegisterShader( "sprites/muted" );
	...
	cgs.media.colorConstructionShader =			trap_R_RegisterShader( "etpro/color_construction" );
...
*/
	...
	cgs.media.ccMortarTargetCM =			trap_R_RegisterShaderNoMip( "gfx/limbo/mort_target_cm" );
	...
	cgs.media.gamestateCorner      = trap_R_RegisterShaderNoMip( "gfx/hud/gamestate/corner" );
	cgs.media.gamestateCornerCount = trap_R_RegisterShaderNoMip( "gfx/hud/gamestate/cornerCount" );
...
//	cgs.media.hflakWeaponSnd =		trap_S_RegisterSound( "sound/vehicles/misc/20mm_fire.wav", qfalse );
	...
	cgs.media.limboClassButton2Wedge_off_5 =	trap_R_RegisterShaderNoMip( "gfx/limbo/skill_4pieces_off_5"		);
	cgs.media.limboClassButton2Wedge_on_5 =		trap_R_RegisterShaderNoMip( "gfx/limbo/skill_4pieces_on_5"		);
...
static int CG_PlayCinematic(const char *name, float x, float y, float w, float h) {
  return trap_CIN_PlayCinematic(name, int(x), int(y), int(w), int(h), CIN_loop);
}
...
//	int startat = trap_Milliseconds();
	...
	/* ANTIHACK.
     * We purposely scramble the order of memsets here so they can't play simple memset trap
     * games to dynamically find these important structure addresses, and add some bogus memset calls.
     */

    memset( &i, 0, sizeof(i) ); // bogus-call for ANTIHACK
	memset( cg_items, 0, sizeof(cg_items) );
	memset( cg_weapons, 0, sizeof(cg_weapons) );
	...
	cgs.media.hitVolumeShader      = trap_R_RegisterShaderNoMip( "sprites/hitVolume" );
	cgs.media.hitVolumeGroupShader = trap_R_RegisterShaderNoMip( "sprites/hitVolumeGroup" );
	cgs.media.hitVolumeHitShader   = trap_R_RegisterShaderNoMip( "sprites/hitVolumeHit" );
...
//	CG_Printf("Time taken: %i\n", trap_Milliseconds() - startat);
    ...
    // Back up the rate for a restore at end
    trap_Cvar_Set("cg_savedRate", cvars::cl_rate.svalue);

    // Allow rate adjustment
    if (cvars::cl_rate.ivalue >= 25000)
        cg.allowIncrease = qtrue;
    CG_AdjustRate();
}
...
void CG_Shutdown( void ) {
    ...
    if (cg_savedRate.integer) {
        trap_Cvar_Set("rate", cg_savedRate.string);
        trap_Cvar_Set("cg_savedRate", "");
    }
}
