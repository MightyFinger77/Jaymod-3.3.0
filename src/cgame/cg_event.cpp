...
// Let each debug tool choose how long its line remains visible.
static void CG_DebugLine(vec3_t start, vec3_t end, vec3_t color, int msec)
{
	localEntity_t	*le;
	refEntity_t		*re;
...
// Mark the muzzle with a cross because the line is hard to see end-on.
static void CG_DebugBulletMarker(vec3_t start, vec3_t end, vec3_t color, int msec)
{
	vec3_t	dir, right, up, a, b;

	CG_DebugLine( start, end, color, msec );
	...
	VectorMA( start, -DEBUG_BULLET_TICK, right, a );
	VectorMA( start,  DEBUG_BULLET_TICK, right, b );
	CG_DebugLine( a, b, color, msec );

	VectorMA( start, -DEBUG_BULLET_TICK, up, a );
	VectorMA( start,  DEBUG_BULLET_TICK, up, b );
	CG_DebugLine( a, b, color, msec );
}
...
static void CG_KillingSpree(int level, int client, int kills) {
	if (!cg_killspreesounds.integer)
		return;

	// Check bounds on level
	if (level < 0 || level >= 6)
		return;

	CG_KillSpreeMessages(cgs.clientinfo[client].name, kills, ks_messages[level]);
	trap_S_StartSoundExVControl(NULL, cg.snap->ps.clientNum, CHAN_VOICE_BG, cgs.media.killingSpree[level], SND_CUTOFF_ALL, 255);
}
...
static void CG_LosingSpree(int level, int client, int kills) {
	...
	CG_KillSpreeMessages(cgs.clientinfo[client].name, -kills, ls_messages[level]);
	trap_S_StartSoundExVControl(NULL, cg.snap->ps.clientNum, CHAN_VOICE_BG, cgs.media.losingSpree[level], SND_CUTOFF_ALL, 255);
}
...
static void CG_Obituary( entityState_t *ent ) {
	...
	// Jaybird - Obituary Filter
	if( cg_obituaryFilter.integer == 2)
		return;
	else if( cg_obituaryFilter.integer == 1 && (!ca || ca->clientNum != cg.clientNum) && ci->clientNum != cg.clientNum )
		return;
	...
	if( attacker == target ) {
		...
		case MOD_SMOKEGRENADE: // rain
			...
			break;
		case MOD_REFLECTED_FF: // Jaybird
			message = "got a taste of his own friendly fire.";
			break;
		// no obituary message if changing teams
		case MOD_SWITCHTEAM:
			return;
		case MOD_POISON_GAS:
			message = "choked on his own poison gas";
			break;
		case MOD_MOLOTOV:
	...
	if (message) {
		...
		// Jaybird - Obituary Location
		switch(cg_obituaryLocation.integer) {
			case OBIT_CHAT:
				CG_AddToTeamChat(va("%s ^7%s.",ci->name, message), -1, 5);
				CG_Printf("%s\n", va("%s ^7%s.",ci->name, message));
				break;
			case OBIT_CPM:
			default:
				CG_AddPMItem( PM_DEATH, va( "%s %s.", targetName, message ), deathShader, flags );
				break;
		}
		return;
	}
	...
	if( ca ) {
		...
		case MOD_KNIFE:
			...
			if( attacker == cg.snap->ps.clientNum || target == cg.snap->ps.clientNum ) {
				trap_S_StartSound( cg.snap->ps.origin, cg.snap->ps.clientNum, CHAN_AUTO, cgs.media.goat );
			}
			break;
		...
		case MOD_GOOMBA: // Jaybird
			message = "got stomped on by";
			message2 = "";
			break;

		case MOD_POISON_SYRINGE:
			message = "took a drink of";
			message2 = "'s poison";
			break;

		case MOD_THROWING_KNIFE:
			message = "got";
			message2 = "'s throwing knife lodged into him";
			break;
		...
		if (message) {
			...
			if ( message2 ) {
				...
				switch(cg_obituaryLocation.integer) {
					case OBIT_CHAT:
						CG_AddToTeamChat(va("%s%s ^7%s %s^7%s", teamkill, targetName, message, attackerName, message2 ), -1, 5);
						CG_Printf("%s\n", va("%s%s ^7%s %s^7%s", teamkill, targetName, message, attackerName, message2 ));
						break;
					case OBIT_CPM:
					default:
						CG_AddPMItem( PM_DEATH, va( "%s%s %s %s%s", teamkill, targetName, message, attackerName, message2 ), deathShader, flags );
						break;
				}
//				CG_Printf( "[cgnotify]%s %s %s%s\n", targetName, message, attackerName, message2 );
	...
	switch(cg_obituaryLocation.integer) {
		case OBIT_CHAT:
			CG_AddToTeamChat(va("%s ^7died.", targetName), -1, 5);
			break;
		case OBIT_CPM:
		default:
			CG_AddPMItem( PM_DEATH, va( "%s died.", targetName ), deathShader, flags );
			break;
	}
}
...
static void CG_ItemPickup( int itemNum ) {
	...
	CG_AddPMItem( PM_MESSAGE, va( "Picked up %s", CG_PickupItemText( itemNum ) ), cgs.media.pmImages[PM_MESSAGE], 0 );
...
//			inheritmodel = cent->currentState.modelindex;
	...
	if( !cent->currentState.dl_intensity ) {
		...
		sound = sfxHandle_t( random()*fxSounds[cent->currentState.frame].max );
...
void CG_RubbleFx(vec3_t origin, vec3_t dir, int mass, int type, sfxHandle_t sound, int forceLowGrav, qhandle_t shader, float speedscale, float sizescale) {
	...
	for(i=0;i<POSSIBLE_PIECES;i++) {
		...
		for(howmany = 0; howmany < pieces[i]; howmany++) {
			...
			le->endTime				= int( (le->startTime + 5000 + random() * 5000) + endtime );
...
void CG_Shard(centity_t *cent, vec3_t origin, vec3_t dir)
{
	...
	for(i = 0;i < howmany; i++)
	{
		...
		le->leType				= LE_FRAGMENT;
		le->startTime			= cg.time;
		le->endTime				= int( le->startTime + 5000 + random() * 5000 );
...
// jpw
	...
	case EV_M97_PUMP:
		...
		// Jaybird - I used the spindown slot for pump sound
		trap_S_StartSound (NULL, es->number, CHAN_WEAPON, cg_weapons[es->weapon].reloadFastSound );
		break;
...
// jpw
	...
	case EV_WEAPONSWITCHED:
		...
		if((es->weapon != WP_GRENADE_LAUNCHER) &&
			...
			(es->weapon != WP_LANDMINE_BBETTY) &&
			(es->weapon != WP_LANDMINE_PGAS) &&
...
/*	case EV_FLAMEBARREL_BOUNCE:
	...
	case EV_RAILTRAIL:
        if (!cg.renderingThirdPerson && es->otherEntityNum2 == cg.snap->ps.clientNum)
            break;
	...
	case EV_FX_SOUND:
		{
			...
			sound = sfxHandle_t( random()*fxSounds[ es->eventParm ].max );
...
//		trap_S_StopStreamingSound( es->number );
		trap_S_StartSoundEx( NULL, es->number, CHAN_VOICE_BG, 0, SND_CUTOFF_ALL );	// kill weapon sound (could be reloading)
		break;
	...
	case EV_GUNSPARKS:
		{
			...
			speed = int( cent->currentState.angles2[2] );
	...
	case EV_POPUPMESSAGE:
		{
			...
			if( str ) {
				CG_AddPMItem( (popupMessageType_t)cent->currentState.effect1Time, str, shader, 0 );
			}
	...
	case EV_ARTYMESSAGE:
		{
			...
			if( wav ) {
				CG_SoundPlaySoundScript( wav, NULL, -1, /*(es->effect1Time ? qfalse : qtrue)*/ qfalse );
			}
		}
		break;
	...
	case EV_BOT_DEBUG_LINE:
		...
		CG_DebugLine(es->origin2, es->pos.trBase, es->angles2, 2000);
		break;

	// pos.trBase is the muzzle and origin2 is the trace endpoint.
	case EV_DEBUG_BULLET:
		DEBUGNAME("EV_DEBUG_BULLET");
		CG_DebugBulletMarker(es->pos.trBase, es->origin2, es->angles2, DEBUG_BULLET_TIME);
		break;
	...
	case EV_ALT_ATTACK2:
		// Skip event if not meant for our client.
		if (cg.clientNum != es->eventParm)
			break;

		// Support weapalt compatibility for non-ATTACK2 weapons.
	...
	case EV_THROW_KNIFE:
		trap_S_StartSound( NULL, es->number, CHAN_WEAPON, cgs.media.throwKnife_sound );
		break;

	case EV_THROW_MOLOTOV:
		trap_S_StartSound( NULL, es->number, CHAN_WEAPON, cgs.media.throwMolotov_sound );
		break;

	case EV_COUGH:
		trap_S_StartSound( 0, es->number, CHAN_BODY, cgs.media.poisonGasCough );
		break;

	case EV_LANDMINE_LAUNCH:
		trap_S_StartSound( 0, es->number, CHAN_WEAPON, cgs.media.landmineLaunch );
		break;
	...
	case EV_FIRSTBLOOD:
		...
		if (!cg_killspreesounds.integer)
			break;
		trap_S_StartSoundExVControl(NULL, cg.snap->ps.clientNum, CHAN_VOICE_BG, cgs.media.firstblood, SND_CUTOFF_ALL, 255);
		break;
	...
	case EV_HEADSHOT:
		...
		if (!cg_killspreesounds.integer)
			break;
		trap_S_StartSoundExVControl(NULL, cg.snap->ps.clientNum, CHAN_VOICE_BG, cgs.media.headshot, SND_CUTOFF_ALL, 255);
		break;
