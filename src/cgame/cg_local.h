...
#ifndef CGAME_CG_LOCAL_H
#define CGAME_CG_LOCAL_H
...
typedef struct centity_s {
    ...
    int 			_ANTIHACK_PADDING[7]; // Change sizeof(centity_t) arbitrarily between releases.
...
typedef struct {
	...
	// Jaybird - killing sprees
	int			ksPrintTime;
	int			ksType;
	char		ksPlayer[MAX_NAME_LENGTH];
	int			ksNumber;
	char		ksMessage[150];
...
//unlagged - optimized prediction
	...
	// Jaybird - grabbed from server, 30000 by default
	int				dynamiteTime;

	// Jaybird - Shoutcaster items
	scItem_t		scItems[MAX_SCITEMS]; // [MAX_SCITEMS]
	int				numSCItems;

	// Jaybird
	qboolean		authenticated;

    // Jaybird - optimized prediction averages
    int numPredicted;
    int numPlayedBack;
    int numSinglePredicted;
    ...
    // Auto rate
    qboolean requestRateIncrease;
    int      rateIncreaseTime;
...
typedef struct {
    qhandle_t	consoleFontShader;
    qhandle_t	consoleFontShadowedShader;
	...
	qhandle_t	hitVolumeShader;
	qhandle_t	hitVolumeGroupShader;
	qhandle_t	hitVolumeHitShader;
...
// jpw
	...
	qhandle_t	polygonCubeCO;  // 1x1x1 unit cube w/ corner as origin
	qhandle_t	polygonCubeFO;  // 1x1x1 unit cube w/ face-center as origin
	qhandle_t	polygonCubeOO;  // 1x1x1 unit cube w/ object-center as origin

	// Jaybird - Gibs
	qhandle_t	gibModels[10];
	...
	// Jaybird new disconnect icon
	qhandle_t	poisonShader;
...
//----(SA)	cursor hints
    ...
    qhandle_t   commandCentreMapMask;
...
//	qhandle_t	shardCeramic2;
	...
	// Jaybird
	qhandle_t	colorConstructionShader;
...
//	sfxHandle_t	sparkSounds;
	...
	// Jaybird - killing spree and multikill
	sfxHandle_t	multikill[7];
	sfxHandle_t killingSpree[6];
	sfxHandle_t	losingSpree[6];
	sfxHandle_t	firstblood;
	...
	sfxHandle_t hitsound_head;
	sfxHandle_t hitsound_hand;
	sfxHandle_t hitsound_torso;
	sfxHandle_t hitsound_foot;

	sfxHandle_t hitsound_fhead;   // friendly
	sfxHandle_t hitsound_fhand;   // friendly
	sfxHandle_t hitsound_ftorso;  // friendly
	sfxHandle_t hitsound_ffoot;   // friendly

	sfxHandle_t throwKnife_sound;
	sfxHandle_t throwMolotov_sound;

	// Jaybird - heartbeat
	sfxHandle_t	heartbeat;
	sfxHandle_t poisonGasCough;
	sfxHandle_t	landmineClick;
	sfxHandle_t	landmineLaunch;

    sfxHandle_t molotovBonk;
    sfxHandle_t molotovScream[5];

	// Jaybird - Watermark
	qhandle_t	watermark;
	int			watermarkFadeAfter;
	int			watermarkFadeTime;
...
//	qhandle_t	hflakWeaponSnd;
	...
	qhandle_t		limboClassButton2Wedge_on_5;	// Jaybird
	qhandle_t		limboClassButton2Wedge_off_5;	// Jaybird
...
//	skill_testicon
	...
	qhandle_t		gamestateCorner;
	qhandle_t		gamestateCornerCount;
...
typedef struct {
    ...
    qhandle_t      teamChatMsgIcons[TEAMCHAT_HEIGHT];
	int				teamChatPos;
	int				teamLastChatPos;
...
extern	vmCvar_t		cg_cycleBinoculars;
...
extern	vmCvar_t		cg_killspreesounds;
extern	vmCvar_t		cg_pmsounds;
extern	vmCvar_t		cg_obituaryLocation;
extern	vmCvar_t		cg_obituaryFilter;
extern	vmCvar_t		cg_watermarkOpacity;
extern	vmCvar_t		cg_althud;
extern	vmCvar_t		cg_drawDisconnectIcon;
extern	vmCvar_t		cg_popupTime;
extern	vmCvar_t		cg_popupFadeTime;
extern	vmCvar_t		cg_popupWaitTime;
extern	vmCvar_t		cg_numPopups;
extern	vmCvar_t		cg_drawClock;
extern	vmCvar_t		cg_muzzleFlash;
extern  vmCvar_t        cg_mapZoom;

extern  vmCvar_t         cg_locationMode;
extern  vmCvar_t         cg_locationMaxChars;
extern  vmCvar_t         cg_locationJustify;

extern  vmCvar_t        cg_drawSpeed;
extern  vmCvar_t        cg_speedRefresh;

extern  vmCvar_t        cg_autoRate;
extern  vmCvar_t        cg_savedRate;
...
qboolean CG_GetWeaponTag( int clientNum, char *tagname, orientation_t* );
...
qboolean CG_HasJayFlag( centity_t *cent, int set, int flag );
...
void CG_Trace_World( trace_t *result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int skipNumber, int mask );
...
//Jaybird - cg_obituaryLocation defines
#define		OBIT_CHAT		1
#define		OBIT_CPM		0
...
// Ridah
void CG_ParticleExplosion (char *animStr, const vec3_t origin, const vec3_t vel, int duration, int sizeStart, int sizeEnd, qboolean dlight );
...
#define CPri(x) CG_CenterPrint(CG_LocalizeServerCommand(x), int(SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.2f)), SMALLCHAR_WIDTH);
...
void CG_AttachBitsToTank( centity_t* tank, refEntity_t* mg42base, refEntity_t* mg42upper, refEntity_t* mg42gun, refEntity_t* player, refEntity_t* flash, const vec3_t playerangles, const char* tagName, qboolean browning );
...
typedef deque<mapEntityData_t*> mapEntList_t;
...
void CG_AddPMItem( popupMessageType_t type, const char* message, qhandle_t shader, int flags );
...
#define FTLOC_NOPVS  0x00000004
#define FTLOC_UNKWN  0x00000008

///////////////////////////////////////////////////////////////////////////////

#include <cgame/cg_molotov.h>

///////////////////////////////////////////////////////////////////////////////

#include <cgame/Font.h>
#include <cgame/Console.h>
#include <cgame/CBulletVolume.h>
#include <cgame/CHitVolume.h>
#include <cgame/Location.h>
#include <cgame/LocationDB.h>

#include <cgame/cvar/public.h>

///////////////////////////////////////////////////////////////////////////////

#endif // CGAME_CG_LOCAL_H
