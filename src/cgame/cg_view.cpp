...
// letterbox is yy:yy  (85% of 'normal' height)
	if(cg_letterbox.integer) {
		...
		if( !center ) {
			int offset = int( (cgs.glconfig.vidHeight*(.5f * lbdiff))/100 );
			offset &= ~1;
			cg.refdef.y += offset;
		}
...
//		}
	...
	cg.refdef.width = int( cgs.glconfig.vidWidth*xsize/100 );
	cg.refdef.width &= ~1;

	cg.refdef.height = int( cgs.glconfig.vidHeight*ysize/100 );
	cg.refdef.height &= ~1;
...
//zinx - fix for #423 - don't sanity check while following
			if (!((cg.snap->ps.pm_flags & PMF_FOLLOW) || cg.demoPlayback)) {
				// Clear scope state left behind after following another player.
				if( cg.snap->ps.weapon && !COM_BitCheck( cg.snap->ps.weapons, cg.weaponSelect ) ) {
...
static int CG_CalcFov( void ) {
	...
	if ( (!(cvars::bg_poisonSyringes.ivalue & POISON_NODISORIENT) && cg.snap->ps.eFlags & EF_POISON) || (contents & ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA )) ){
		phase = cg.time / 1000.0 * WAVE_FREQUENCY * M_PI * 2;
		v = WAVE_AMPLITUDE * sin( phase );

		// Jaybird - add some for poison
		if(!(cvars::bg_poisonSyringes.ivalue & POISON_NODISORIENT) && cg.snap->ps.eFlags & EF_POISON)
			v *= 4;
...
int CG_CalcViewValues( void ) {
	...
	if( !cg.showGameView ) {
		...
		if( !(cvars::bg_poisonSyringes.ivalue & POISON_NODISORIENT) && ps->eFlags & EF_POISON ) {
			cg.refdefViewAngles[ROLL] = sin((float)cg.time/1000.f) * 16.f;
			cg.refdefViewAngles[PITCH] += sin((float)cg.time/500.f) * 5.f;
			cg.refdefViewAngles[YAW] += sin((float)cg.time/1000.f) * 16.f;
		}
