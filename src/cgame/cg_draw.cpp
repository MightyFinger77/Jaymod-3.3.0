	...
	if( cgs.teamLastChatPos != cgs.teamChatPos ) {
		...
		for( i = cgs.teamChatPos - 1; i >= cgs.teamLastChatPos; i-- ) {
            ...
            if (cgs.teamChatMsgIcons[i % chatHeight] >= 0) {
                CG_DrawPic( CHATLOC_X, CHATLOC_Y - (cgs.teamChatPos - i)*lineHeight, 10, 8, cgs.teamChatMsgIcons[i % chatHeight]);
            }

			CG_Text_Paint_Ext( CHATLOC_TEXT_X + 10, CHATLOC_Y - (cgs.teamChatPos - i - 1) * lineHeight - 2, 0.2f, 0.2f, hcolor, cgs.teamChatMsgs[i % chatHeight], 0, 0, 0, &cgs.media.limboFont2 );
		}
	}
}
...
static void CG_DrawLagometer( void ) {
	...
	// Draw the intended 48 samples and scale them to the current resolution.
	const int   LAG_GRAPH_SAMPLES = 48;
	const float sw = aw / (float)LAG_GRAPH_SAMPLES;

	// draw the frame interpoalte / extrapolate graph
	for ( a = 0 ; a < LAG_GRAPH_SAMPLES ; a++ ) {
		...
		if ( v > 0 ) {
			...
			trap_R_DrawStretchPic ( ax + aw - (a+1)*sw, mid - v, sw, v, 0, 0, 0, 0, cgs.media.whiteShader );
		} else if ( v < 0 ) {
			if ( color != 2 ) {
				color = 2;
			...
			trap_R_DrawStretchPic( ax + aw - (a+1)*sw, mid, sw, v, 0, 0, 0, 0, cgs.media.whiteShader );
		}
	}
	...
	for ( a = 0 ; a < LAG_GRAPH_SAMPLES ; a++ ) {
		...
		if ( v > 0 ) {
            ...
            if (lagometer.snapshotAntiwarp[i] > 0) {
                ...
                trap_R_DrawStretchPic( ax + aw - (a+1)*sw, ay + ah - w - 2, sw, w, 0, 0, 0, 0, cgs.media.whiteShader );
            }
			...
			trap_R_DrawStretchPic( ax + aw - (a+1)*sw, ay + ah - v - 2, sw, v, 0, 0, 0, 0, cgs.media.whiteShader );
		} else if ( v < 0 ) {
			if ( color != 4 ) {
				color = 4;		// RED for dropped snapshots
				trap_R_SetColor( colorRed );
			}
			trap_R_DrawStretchPic( ax + aw - (a+1)*sw, ay + ah - range - 2, sw, range, 0, 0, 0, 0, cgs.media.whiteShader );
		}
	}
...
static void CG_DrawCrosshair(void) {
	...
	// Keep crosshair offsets in screen pixels while scaling its size.
	x = y = 0;
	CG_AdjustFrom640( &x, &y, &w, &h );

	hShader = cgs.media.crosshairShader[ cg_drawCrosshair.integer % NUM_CROSSHAIRS ];

	trap_R_DrawStretchPic( cg_crosshairX.integer + 0.5 * (cg.refdef_current->width - w), cg_crosshairY.integer + 0.5 * (cg.refdef_current->height - h), w, h, 0, 0, 1, 1, hShader );
	...
	if ( cg.crosshairShaderAlt[ cg_drawCrosshair.integer % NUM_CROSSHAIRS ] ) {
		...
		trap_R_DrawStretchPic( cg_crosshairX.integer + 0.5 * (cg.refdef_current->width - w), cg_crosshairY.integer + 0.5 * (cg.refdef_current->height - h), w, h, 0, 0, 1, 1, cg.crosshairShaderAlt[ cg_drawCrosshair.integer % NUM_CROSSHAIRS ] );
	}
}
...
static void CG_DrawNoShootIcon( void ) {
	...
	// Match the crosshair's screen-pixel offset.
	x = y = 0;
	CG_AdjustFrom640( &x, &y, &w, &h );

	// FIXME precache
	trap_R_DrawStretchPic( (cg_crosshairX.integer + 1) + 0.5 * (cg.refdef_current->width - w), (cg_crosshairY.integer + 1) + 0.5 * (cg.refdef_current->height - h), w, h, 0, 0, 1, 1, cgs.media.friendShader );
}
