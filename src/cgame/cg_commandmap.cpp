...
/* Avoid iostream facets, which can fail when ET loads this library. */
static const char* CG_ParseIntBase( const char* p, int* out, int base ) {
	char* end = NULL;
	long v;
	if ( !p || !*p ) {
		*out = 0;
...
static const char* CG_ParseDec( const char* p, int* out ) {
	return CG_ParseIntBase( p, out, 10 );
}

static const char* CG_ParseHex( const char* p, int* out ) {
	return CG_ParseIntBase( p, out, 16 );
}

static const char* CG_ParseMapEntity( const char* entList, team_t team ) {
	mapEntityData_t* mEnt = new mapEntityData_t;
	memset( mEnt, 0, sizeof( *mEnt ) );

	entList = CG_ParseDec( entList, &mEnt->type );

	switch ( mEnt->type ) {
	case ME_PLAYER:
	case ME_PLAYER_REVIVE:
	case ME_PLAYER_DISGUISED:
		entList = CG_ParseHex( entList, &mEnt->yaw );
	...
	case ME_TANK_DEAD:
	case ME_LANDMINE:
		entList = CG_ParseDec( entList, &mEnt->x );
		mEnt->x *= 128;
		entList = CG_ParseDec( entList, &mEnt->y );
		mEnt->y *= 128;
		if ( cgs.ccLayers ) {
			entList = CG_ParseDec( entList, &mEnt->z );
			mEnt->z *= 128;
		}
		break;
	...
	entList = CG_ParseDec( entList, &mEnt->data );
...
void CG_ParseMapEntityInfo( int axis_number, int allied_number, bool reset ) {
	...
	/* Use the parser above to avoid the iostream loader issue. */
	entList = CG_Argv( 3 );

	for ( i = 0; i < axis_number; i++ ) {
		entList = CG_ParseMapEntity( entList, TEAM_AXIS );
	}

	for ( i = 0; i < allied_number; i++ ) {
		entList = CG_ParseMapEntity( entList, TEAM_ALLIES );
	}
