...
//	if ( CG_PointContents( re->origin, -1 ) & CONTENTS_WATER ) {
	...
	
	{
		...
		else if( cent->currentState.weapon == WP_M97 )
			...
			if( ent->currentState.teamNum < 8 ) {
				...
				if( !ent->miscTime ) {
					...
					switch (ent->currentState.weapon) {
						case WP_LANDMINE_BBETTY:
						case WP_LANDMINE_PGAS:
							trap_S_StartSound( NULL, ent->currentState.number, CHAN_WEAPON, cgs.media.landmineClick );
							break;
...
==========================
CG_RailTrail3
	Jaybird - a modified CG_RailTrail2 for colored hitboxes
==========================
*/
void CG_RailTrail3( clientInfo_t *ci, vec3_t start, vec3_t end, int type) {
	localEntity_t	*le;
	refEntity_t		*re;
	...
	le->startTime = cg.time;
	le->endTime = cg.time + ( 1000 / cvars::sv_fps.ivalue );
	le->lifeRate = 1.0 / ( le->endTime - le->startTime );
	...
	if( type == 1 ) {       // Red for etmain box
		le->color[0] = 1.0f;
		le->color[1] = le->color[2] = 0;
...
void CG_RailTrail( clientInfo_t *ci, vec3_t start, vec3_t end, int type) {	//----(SA)	added 'type'
	...
	v2[1] -= diff[1];
	v3[2] -= diff[2];
	CG_RailTrail3( ci, start, v1, type);
	CG_RailTrail3( ci, start, v2, type);
	CG_RailTrail3( ci, start, v3, type);
	...
	v5[1] += diff[1];
	v6[2] += diff[2];
	CG_RailTrail3( ci, end, v4, type);
	CG_RailTrail3( ci, end, v5, type);
	CG_RailTrail3( ci, end, v6, type);

	CG_RailTrail3( ci, v2, v6, type);
	CG_RailTrail3( ci, v6, v1, type);
	CG_RailTrail3( ci, v1, v5, type);

	CG_RailTrail3( ci, v2, v4, type);
	CG_RailTrail3( ci, v4, v3, type);
	CG_RailTrail3( ci, v3, v5, type);
...
//					CG_Error( "Couldn't register weapon %i (failed to parse %s)", weaponNum, filename );
		...
		} else if( !Q_stricmp( token.string, "scaleFirstPerson" ) ) {
				if( !PC_Float_Parse( handle, &weaponInfo->scaleFirstPerson ) ) {
					return CG_RW_ParseError( handle, "expected scaleFirstPerson value" );
				}
...
void CG_RegisterWeapon( int weaponNum, qboolean force ) {
	...
	switch( weaponNum ) {
		...
		case WP_ADRENALINE_SHARE:		filename = "adrenaline_share.weap"; break;
		...
		case WP_POISON_GAS:				filename = "poison_gas.weap"; break;
...
static void CG_CalculateWeaponPosition( vec3_t origin, vec3_t angles ) {
	...
	if(cg.predictedPlayerState.leanf != 0) {
		...
		angles[PITCH] += abs( int(cg.predictedPlayerState.leanf) ) / 2.0f;
...
//	qboolean	playerScaled;
	...
	// Jaybird - Don't draw the weapon when playing dead...
	if (ps && (ps->eFlags & EF_PLAYDEAD))
		return;
	...
	if( BG_IsAkimboWeapon( weaponNum ) ) {
		if( isPlayer ) {
			akimboFire = BG_AkimboFireSequence( weaponNum, cg.predictedPlayerState.ammoclip[BG_FindClipForWeapon( (weapon_t)weaponNum )], cg.predictedPlayerState.ammoclip[BG_FindClipForWeapon( (weapon_t)BG_AkimboSidearm(weaponNum) )] );
		} else if( ps ) {
			akimboFire = BG_AkimboFireSequence( weaponNum, ps->ammoclip[BG_FindClipForWeapon( (weapon_t)weaponNum )], ps->ammoclip[BG_FindClipForWeapon( (weapon_t)BG_AkimboSidearm(weaponNum) )] );
		}
		...
		for( i = W_PART_1; i < W_MAX_PARTS; i++ ) {
			...
			if( barrel.hModel ) {
				...
				if( drawpart ) {
					...
					if( (weaponNum == WP_POISON_SYRINGE || weaponNum == WP_ADRENALINE_SHARE) && i == W_PART_1) {
						barrel.customShader = weapon->modModels[ 0 ];
					}
...
//				CG_ParticleImpactSmokePuff (cgs.media.smokeParticleShader, flash.origin);
	...
	if( flash.hModel ) {
		if(weaponNum != WP_FLAMETHROWER ) {	//Ridah, hide the flash also for now
			// Jaybird - disable muzzle flash
			if (!cg_muzzleFlash.integer && ps && !cg.renderingThirdPerson)
				return;
	...
	if ( ps || cg.renderingThirdPerson || !isPlayer )
	{
		...
		if( firing && !(cent->currentState.eFlags & (EF_PRONE_MOVING | EF_DEAD | EF_PLAYDEAD)) )
		{
...
//----(SA)	removed
		...
		// Jaybird - 1st person model scale
		if ( weapon->scaleFirstPerson ) {
			hand.axis[0][0] *= weapon->scaleFirstPerson;
			hand.axis[0][1] *= weapon->scaleFirstPerson;
			hand.axis[0][2] *= weapon->scaleFirstPerson;
		}
...
Returns true when binoculars should be omitted from weapon cycling.
==============
*/
static qboolean CG_CycleSkipWeap( int num ) {
	return (qboolean)( num == WP_BINOCULARS && !cg_cycleBinoculars.integer );
}
...
void CG_WeaponBank_f(void) {
	...
	if(!cg.lastWeapSelInBank[bank]) {
		...
		// Jaybird - smg on slot 2 for heavy weaps
		CG_WeaponIndex(num, NULL, &cycle);
		if( bank != curbank ) {
			cycle -= 1;
		}
	}
...
void CG_OutOfAmmoChange( qboolean allowforceswitch ) {
	...
	// Satchel detonator charges are stored in ammoclip, not just ammo.
	if( cg.weaponSelect == WP_PLIERS || ( cg.weaponSelect == WP_SATCHEL_DET &&
		( cg.predictedPlayerState.ammo[WP_SATCHEL_DET] || cg.predictedPlayerState.ammoclip[WP_SATCHEL_DET] ) ) ) {
		return;
	}
		...
		switch (cg.weaponSelect) {
			...
			case WP_PANZERFAUST:
				...
				for( i = 0; i < MAX_WEAPS_IN_BANK_MP; i++ ) {
					...
					if (weapBanksMultiPlayer[3][i] != WP_PANZERFAUST && CG_WeaponSelectable(weapBanksMultiPlayer[3][i])) {
						// find a smg
...
// check for water/dirt spurt
		if (CG_PointContents( origin, 0 ) & CONTENTS_WATER) {
			...
			CG_AddDirtBulletParticles( trace.endpos, dir, int( 400+random()*200 ), 900, 15, 0.5, 512, 128, 0.125, cgs.media.dirtParticle2Shader);
			CG_AddDirtBulletParticles( trace.endpos, dir, int( 400+random()*600 ), 1400, 15, 0.5, 128, 512, 0.125, cgs.media.dirtParticle2Shader);
		} else {
			VectorSet(tmpv, origin[0], origin[1], origin[2] + 20);
			VectorSet(tmpv2, origin[0], origin[1], origin[2] - 20);
			trap_CM_BoxTrace(&trace, tmpv, tmpv2, NULL, NULL, 0, MASK_SHOT);
			if (trace.surfaceFlags & SURF_GRASS || trace.surfaceFlags & SURF_GRAVEL) {
				CG_AddDirtBulletParticles( origin, dir, int( 400+random()*200 ), 3000, 10, 0.5, 400,256, 0.25, cgs.media.dirtParticle1Shader);
			}
 			for (i=0; i<3; i++) {
				for (j=0;j<3;j++) {
			...
			CG_AddDebris( origin, dir, int( 400 + random()*200 ), rand()%2000 + 1400, 12 + rand()%12 );
		}
		break;
	case WP_GRENADE_LAUNCHER:
	case WP_GRENADE_PINEAPPLE:
	...
	case WP_SMOKE_MARKER:
		...
		if (CG_PointContents( origin, 0 ) & CONTENTS_WATER) {
			...
			CG_AddDebris( origin, dir, int( 400+random()*200 ), rand()%2000+1000, 5 + rand()%5 );
		}
		break;
...
// jpw
		...
		if( CG_CalcMuzzlePoint( sourceEntityNum, start ) || cg.snap->ps.persistant[PERS_HWEAPON_USE]) {
			if( waterfraction ) {
				...
				CG_MissileHitWall( fromweap, 1, end, normal, 0 );
			} else {
				VectorSubtract( end, start, dir );
				VectorNormalizeFast( dir );
				VectorMA( end, 4, dir, end );

				CG_Trace( &trace, start, NULL, NULL, end, 0, MASK_SHOT & ~CONTENTS_BODY);
				/*
...
static void CG_M97Pattern( vec3_t origin, vec3_t origin2, int seed, int otherEntNum ) {
	int			i;
	vec3_t		end;
	vec3_t		forward, right, up;
	...
	for ( i = 0 ; i < M97_COUNT ; i++ ) {
        ...
        qboolean flesh = cg_entities[tr.entityNum].currentState.eType == ET_PLAYER ? qtrue : qfalse;

		CG_Bullet( tr.endpos, otherEntNum, tr.plane.normal, flesh, tr.entityNum, 0, waterfraction, 0);
	}
}

void CG_M97Fire( entityState_t *es ) {
	vec3_t	v;
	...
	CG_M97Pattern( es->pos.trBase, es->origin2, es->eventParm, es->otherEntityNum );
}
