...
void CG_ParseJaymodinfo( void) {
    ...
    cvars::bg_aimSpreadTurnScale.set( Info_ValueForKey( info, "y" ));
...
// cg_noVoiceChats may keep team voice enabled while muting global voice.
#define VOICECHAT_MUTE			1
#define VOICECHAT_ALLOWTEAM		2
...
*/
	...
	// Team and fireteam voice may be allowed separately.
	if ( !( cg_noVoiceChats.integer & VOICECHAT_MUTE ) ||
	     ( ( cg_noVoiceChats.integer & VOICECHAT_ALLOWTEAM ) &&
	       ( vchat->mode == SAY_TEAM || vchat->mode == SAY_BUDDY ) ) ) {
