...
void CG_Debriefing_ChatBox_Draw( panel_button_t* button ) {
	...
	int chatWidth = int(button->rect.w);
	int chatHeight = int(button->rect.h);
	...
	switch( cgs.dbMode ) {
		...
		case 3:
			...
			if( button && button->onDraw == CG_Debriefing_Scrollbar_Draw ) {
				...
				count = int((cnt * button->data[1] * 0.5f) / (float)(r.h));
				if( count ) {
					...
					if(ofs == count) {
						button->data[1] -= ofs * int(r.h / (float)cnt);
					}
				}

				CG_Debriefing_ScrollGetBarRect( button, &r );
				cgs.cursorY = int(r.y + button->data[2]);
...
qboolean CG_Debriefing_Scrollbar_KeyDown( panel_button_t* button, int key ) {
	if( key == K_MOUSE1 ) {
		...
		if( BG_CursorInRect( &r ) ) {
			...
			button->data[1] = 0;
			button->data[2] = int(cgs.cursorY - r.y);
		}
	}
...
qboolean CG_TeamDebriefingMapList_KeyDown( panel_button_t* button, int key ) {
	if( key == K_MOUSE1 ) {
		int pos	= int(( cgs.cursorY - button->rect.y ) / 12) + cgs.tdbMapListOffset;
...
const char* awardNames[NUM_ENDGAME_AWARDS] = {
	...
	"Best Soldier",					//
	"Highest Efficiency (XP/MIN)",	//
	"Highest Accuracy",				//
	"I Ain't Got No Friends Award",	// min 5 tks
	"Welcome Newbie! Award",		// highest XP/min, except that you dont get this
									// if any other award given or > 100 xp (this map)
};
...
qboolean CG_Debriefing2_Maps_KeyDown( panel_button_t* button, int key ) {
	if( key == K_MOUSE1 ) {
		if( cg_gameType.integer == GT_WOLF_CAMPAIGN ) {
			int pos	= int(( cgs.cursorY - button->rect.y ) / 14) + cgs.tdbMapListOffset;
