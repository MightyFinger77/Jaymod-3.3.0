...
Mixes the 2 colors to a factor.
Factor == 1 means full color 1
==================
*/
void CG_LerpColor2(vec4_t color1, vec4_t color2, vec4_t result, float factor)
{
    ...
    for( int i = 0; i < 4; i++) {
        result[i] = (color1[i] * factor) + (color2[i] * (1.0f - factor));
    }
}
...
Mixes the 3 colors to a factor.
Factor == 1 means full color 1
==================
*/
void CG_LerpColor3(vec4_t color1, vec4_t color2, vec4_t color3, vec4_t result, float factor)
{
    ...
    if (factor >= 0.5f) {
        ...
        for (int i = 0; i < 4; i++) {
            result[i] = (color2[i] * factor) + (color3[i] * (1.0f - factor));
        }
    }
}
	...
	while ( *s && cnt < maxChars) {
		if ( Q_IsColorString( s ) ) {
			if ( !forceColor ) {
				...
				// CHRUKER: b082 - Save the new color so that it can be used on subsequent lines
...
==================
CG_ColorForPercent
Jaybird - Derived from CG_ColorForHealth
==================
*/
