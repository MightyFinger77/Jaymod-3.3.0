...
void CG_mvDraw(cg_window_t *sw)
{
	...
	refdef.x      = int( rd_x );
	refdef.y      = int( rd_y );
	refdef.width  = int( rd_w );
	refdef.height = int( rd_h );
	...
	x = int( refdef.width / tan(refdef.fov_x / 360 * M_PI) );
	refdef.fov_y = atan2(refdef.height, (float)x) * 360 / M_PI;
...
void CG_mvWindowOverlay(int pID, float b_x, float b_y, float b_w, float b_h, float s, int wState, qboolean fSelected)
{
	...
	CG_DrawStringExt( int(b_x + 1), int(b_y + b_h - (fh*2 + 1 + 2)), ci->name, colorWhite, qfalse, qtrue, int(fw), int(fh), 0);
	CG_DrawStringExt( int(b_x + 1), int(b_y + b_h - (fh + 2)), va("%s^7%d", CG_TranslateString(p_class), ci->health), colorWhite, qfalse, qtrue, int(fw), int(fh), 0);

	p_class = va("%d^1/^7%d", ci->ammoclip, ci->ammo);
	x = CG_DrawStrlen(p_class);
	CG_DrawStringExt( int(b_x + b_w - (x * fw + 1)), int(b_y + b_h - (fh + 2)), p_class, colorWhite, qfalse, qtrue, int(fw), int(fh), 0);
	...
	if(ci->sprintTime >= 0) {
		...
		rect.y -= (fh + 1);
		x = CG_DrawStrlen(p_class);
		CG_DrawStringExt( int(b_x + b_w - (x * fw + 1)), int(rect.y), p_class, colorWhite, qfalse, qtrue, int(fw), int(fh), 0);
	}
	...
	if(ci->hintTime >= 0) {
		...
		rect.y -= (fh + 1);
		x = CG_DrawStrlen(p_class);
		CG_DrawStringExt(int(b_x + (b_w - (x * (fw-1))) / 2), int(b_y + b_h - (fh + 2)), p_class, colorWhite, qfalse, qtrue, int(fw)-1, int(fh)-1, 0);
	}
