...
vmMain( intptr_t command, intptr_t arg0, intptr_t arg1, intptr_t arg2, intptr_t arg3, intptr_t arg4, intptr_t arg5, intptr_t arg6, intptr_t arg7, intptr_t arg8, intptr_t arg9, intptr_t arg10, intptr_t arg11  ) {
	...
	case UI_KEY_EVENT:
		_UI_KeyEvent( (int)arg0, (qboolean)arg1 );
		return 0;
	...
	case UI_CHECKEXECKEY:
		return UI_CheckExecKey( (int)arg0 );
...
int Text_Width_Ext( const char *text, float scale, int limit, fontInfo_t* font ) {
	...
	return int( out * scale * font->glyphScale );
}
...
int Text_Height_Ext( const char *text, float scale, int limit, fontInfo_t* font ) {
	...
	return int( max * scale * font->glyphScale );
}
...
int Multiline_Text_Height( const char *text, float scale, int limit ) {
	...
	if( totalheight > 0 ) {
		...
		totalheight += max;
		return int( totalheight * scale * font->glyphScale );
	} else {
		return int( max * scale * font->glyphScale );
	}
}
...
void _UI_Refresh( int realtime )
{
	...
	if(ui_blackout.integer > 0) UI_FillRect(-10, -10, 650, 490, colorBlack);
		...
		if (uiInfo.teamList[i].cinematic >= -2) {
			...
			if (uiInfo.teamList[i].cinematic >= 0) {
			  trap_CIN_RunCinematic(uiInfo.teamList[i].cinematic);
				trap_CIN_SetExtents(uiInfo.teamList[i].cinematic, int(rect->x), int(rect->y), int(rect->w), int(rect->h));
	...
	if (uiInfo.previewMovie > -2) {
		...
		if (uiInfo.previewMovie >= 0) {
		  trap_CIN_RunCinematic(uiInfo.previewMovie);
			trap_CIN_SetExtents(uiInfo.previewMovie, int(rect->x), int(rect->y), int(rect->w), int(rect->h));
...
/*		if( uiInfo.campaignList[map].campaignCinematic >= -1 ) {
	...
	if (uiInfo.mapList[map].cinematic >= -1) {
		...
		if (uiInfo.mapList[map].cinematic >= 0) {
			...
			trap_CIN_SetExtents(uiInfo.mapList[map].cinematic, int(rect->x), int(rect->y), int(rect->w), int(rect->h));
...
static void UI_DrawCampaignCinematic(rectDef_t *rect, float scale, vec4_t color, qboolean net) {
	...
	if (uiInfo.campaignList[campaign].campaignCinematic >= -1) {
		...
		if (uiInfo.campaignList[campaign].campaignCinematic >= 0) {
			...
			trap_CIN_SetExtents(uiInfo.campaignList[campaign].campaignCinematic, int(rect->x), int(rect->y), int(rect->w), int(rect->h));
...
static void UI_DrawPlayerModel(rectDef_t *rect) {
	...
	if (trap_Cvar_VariableValue("ui_Q3Model")) {
		// NERVE - SMF
		int teamval;
		teamval = int( trap_Cvar_VariableValue( "mp_team" ));
	...
	// compare new cvars to old cvars and see if we need to update
	{
		...
		v1 = int( trap_Cvar_VariableValue( "mp_team" ));
		v2 = int( trap_Cvar_VariableValue( "ui_prevTeam" ));
		if ( v1 != v2 ) {
			trap_Cvar_Set( "ui_prevTeam", va( "%i", v1 ) );
			updateModel = qtrue;
		}

		v1 = int( trap_Cvar_VariableValue( "mp_playerType" ));
		v2 = int( trap_Cvar_VariableValue( "ui_prevClass" ));
		if ( v1 != v2 ) {
			trap_Cvar_Set( "ui_prevClass", va( "%i", v1 ) );
			updateModel = qtrue;
		}

		v1 = int( trap_Cvar_VariableValue( "mp_weapon" ));
		v2 = int( trap_Cvar_VariableValue( "ui_prevWeapon" ));
		if ( v1 != v2 ) {
...
//  	VectorClear( moveangles );
    ...
    UI_PlayerInfo_SetInfo( &info, LEGS_IDLE, TORSO_STAND, viewangles, moveangles, (weapon_t)-1, qfalse );
...
static int UI_OwnerDrawWidth(int ownerDraw, float scale) {
  ...
  switch (ownerDraw) {
    ...
    case UI_BLUETEAM1:
		...
		case UI_BLUETEAM4:
		case UI_BLUETEAM5:
			value = int( trap_Cvar_VariableValue(va("ui_blueteam%i", ownerDraw-UI_BLUETEAM1 + 1)) );
			if (value <= 0) {
				text = "Closed";
			} else if (value == 1) {
				text = "Human";
    ...
    case UI_REDTEAM1:
		...
		case UI_REDTEAM4:
		case UI_REDTEAM5:
			value = int( trap_Cvar_VariableValue(va("ui_redteam%i", ownerDraw-UI_REDTEAM1 + 1)) );
			if (value <= 0) {
				text = "Closed";
			} else if (value == 1) {
				text = "Human";
	...
	if (uiInfo.serverStatus.motdLen) {
			...
			if( uiInfo.serverStatus.motdPaintX <= rect->x + 2 ) {
				if( uiInfo.serverStatus.motdOffset < uiInfo.serverStatus.motdLen ) {
					...
					if (uiInfo.serverStatus.motdPaintX2 >= 0) {
						...
						uiInfo.serverStatus.motdPaintX = int( rect->x + rect->w - 2 );
					}
					uiInfo.serverStatus.motdPaintX2 = -1;
				}
			} else {
		...
		if( uiInfo.serverStatus.motdOffset && maxX > 0 ) {
			...
			if (uiInfo.serverStatus.motdPaintX2 == -1) {
				uiInfo.serverStatus.motdPaintX2 = int( rect->x + rect->w - 2 );
			}
		} else {
			uiInfo.serverStatus.motdPaintX2 = -1;
		}
...
//			Menus_ActivateByName( "background_1", qtrue );
			...
			Menus_CloseAll();
		} else if (Q_stricmp(name, "voteGeneric") == 0) {
			trap_Cmd_ExecuteText( EXEC_APPEND, va( "callvote generic \"%s\"\n", UI_Cvar_VariableString( "ui_voteGeneric" ) ) );
			trap_Cvar_Set( "ui_voteGeneric", "" );
		...
		// DHM - Nerve
			...
			// TTimo
			int sv_pb = int( trap_Cvar_VariableValue( "sv_punkbuster" ));
			if (sv_pb)
		...
		else if( Q_stricmp( name, "vidSave" ) == 0 )
		{
			...
			// get mode
			mode = int( trap_Cvar_VariableValue( "r_mode" ));
		...
		else if( Q_stricmp( name, "vidReset" ) == 0 )
		{
			...
			// get old mode
			oldMode = int( trap_Cvar_VariableValue( "r_oldMode" ));
			if( oldMode == 0 )
				oldMode = 3;
        ...
        // Jaybird - shrubbot commands
        } else if ( Q_stricmp( name, "sbSetLevel" ) == 0 ) {
            if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) {
                char levelNum[128];
                trap_Cvar_VariableStringBuffer( "ui_setlevelnum", levelNum, sizeof(levelNum) );
                trap_Cvar_Set( "ui_setlevelnum", "" );

                trap_Cmd_ExecuteText( EXEC_APPEND, va("!setlevel \"%d\" \"%s\"\n",uiInfo.playerClientNums[uiInfo.playerIndex], levelNum) );
            } 
        } else if ( Q_stricmp( name, "sbFinger" ) == 0 ) {
            if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) {
                trap_Cmd_ExecuteText( EXEC_APPEND, va("!finger \"%d\"\n",uiInfo.playerClientNums[uiInfo.playerIndex]) );
            }
        }
	...
	*numhandles = 0;
	if (feederID == FEEDER_HEADS) {
		...
		if( index >= 0 && index < uiInfo.serverStatus.numDisplayServers) {
			int ping, game, antilag, needpass, friendlyfire, maxlives, punkbuster, weaponrestrictions, balancedteams, serverload, mod;
...
void _UI_SetActiveMenu( uiMenuCommand_t menu ) {
  ...
  if (Menu_Count() > 0) {
	  ...
	  case UIMENU_INGAME:
		...
		// Jaybird - private messages
		case UIMENU_INGAME_PRIVATEMESSAGE:
			trap_Key_SetCatcher( KEYCATCH_UI );
