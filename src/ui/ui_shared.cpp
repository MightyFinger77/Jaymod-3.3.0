...
{
	UIRectangle *itemRect = &item->window.rectClient;
	UIRectangle *tipRect = &item->toolTipData->window.rectClient;
...
void Window_Paint(Window *w, float fadeAmount, float fadeClamp, float fadeCycle) {
	...
	if (w->style == WINDOW_STYLE_FILLED) {
	  ...
	  if (w->background) {
		  Fade(&w->flags, &w->backColor[3], fadeClamp, &w->nextTime, int( fadeCycle ), qtrue, fadeAmount);
...
qboolean Item_SettingShow(itemDef_t *item, qboolean fVoteTest)
{
	...
	if(item->settingFlags & SVS_ENABLED_SHOW) return (qboolean)( (atoi(info) & item->settingTest) );
	if(item->settingFlags & SVS_DISABLED_SHOW) return (qboolean)( (!(atoi(info) & item->settingTest)) );
		...
		if( key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2 || key == K_MOUSE3 ) {
			...
			if( !(item->cvarFlags & CVAR_NOTOGGLE) ) {
				if( item->type == ITEM_TYPE_TRICHECKBOX ) {
					int curvalue = int( DC->getCVarValue( item->cvar )) + 1;
					if( curvalue > 2 )
						curvalue = 0;
	...
	switch ( key ) {
		...
		case K_F11:
			if (DC->getCVarValue("developer")) {
				debugMode = debugMode ? qfalse : qtrue;
			}
			break;
...
//	{ "weapalt",	K_MOUSE4,		-1,	K_MOUSE4,		-1,	-1,	-1 },	// Jaybird
	...
	{ "selectbuddy 8",	K_KP_PGUP,		-1,	K_KP_PGUP,		-1,	-1,	-1 },
	...
	if (item->window.flags & WINDOW_HORIZONTAL) {
		...
		if (listPtr->elementStyle == LISTBOX_IMAGE) {
			...
			for (i = listPtr->startPos; i < count; i++) {
				...
				image = DC->feederItemImage(item->special, int(i) );
				if (image) {
		...
		if (listPtr->elementStyle == LISTBOX_IMAGE) {
			...
			for (i = listPtr->startPos; i < count; i++) {
				...
				if (size < listPtr->elementHeight) {
					listPtr->drawPadding = int( size ); //listPtr->elementHeight - size;
					break;
				}
			...
			for (i = listPtr->startPos; i < count; i++) {
				...
				if (listPtr->numColumns > 0) {
					...
					for (j = 0; j < listPtr->numColumns; j++) {
						text = DC->feederItemText(item->special, int(i), int(j), optionalImages, &numOptionalImages);
					...
					text = DC->feederItemText(item->special, int(i), 0, optionalImages, &numOptionalImages);
					if( numOptionalImages >= 0 ) {
...
qboolean Item_ValidateTooltipData(itemDef_t *item)
{
	...
	item->toolTipData = (itemDef_s*)UI_Alloc(sizeof(itemDef_t));
...
{
	return (Item_ValidateTooltipData(item) && PC_String_Parse(handle, &item->toolTipData->text)) ? qtrue : qfalse;
}

qboolean ItemParse_tooltipalignx( itemDef_t *item, int handle )
{
	return (Item_ValidateTooltipData(item) && PC_Float_Parse(handle, &item->toolTipData->textalignx)) ? qtrue : qfalse;
}

qboolean ItemParse_tooltipaligny( itemDef_t *item, int handle )
{
	return (Item_ValidateTooltipData(item) && PC_Float_Parse(handle, &item->toolTipData->textaligny)) ? qtrue : qfalse;
}
