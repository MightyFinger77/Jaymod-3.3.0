...
#ifndef BGAME_PUBLIC_H
#define BGAME_PUBLIC_H

///////////////////////////////////////////////////////////////////////////////

#include <bgame/cvar/public.h>
namespace cvars = cvar::objects;
using cvar::Cvar;

#include <bgame/NewAllocator.h>
#include <bgame/NewAllocator.tcc>
#include <bgame/Engine.h>
#include <bgame/Logger.h>
...
// Limits rapid stand-to-crouch transitions.
#define DUCK_RATE_WINDOW		1000
#define DUCK_RATE_MAX			3
#define DUCK_RATE_MIN_INTERVAL	334
...
#define	PMF_DOUBLEJUMPING	128		// Jaybird - double jumping?
...
typedef struct {
	...
	// Shared prediction state for the crouch rate limit.
	int			duckRateWindow;		// serverTime the current rate window opened
	int			duckRateCount;		// stand->duck transitions counted in that window
	int			lastDuckTime;		// serverTime of the last stand->duck transition
	...
	// Fractional weapon cooling retained between predicted frames.
	float		weapHeatFrac[MAX_WEAPONS];

	// Jaybird - Poison syringes
	poison_t	poisonEvents[MAX_POISONEVENTS];

	int shoveTime;

	a2_alt_t           a2_alt;
	a2_molotov_t       a2_molotov;
	a2_throwingKnife_t a2_throwingKnife;
...
typedef enum {
	...
	STAT_CAPTUREHOLD_RED,			// REMOVE: JPW NERVE - red team score
	STAT_CAPTUREHOLD_BLUE,			// REMOVE: JPW NERVE - blue team score
...
#define EF_POISON 			0x00010000		// Jaybird - poisoned
#define EF_FAKEBMODEL		0x00010000		// Jaybird - overloaded
...
#define EF_PLAYDEAD			0X00800000		// Jaybird - playing dead
...
// Gordon: ONLY secondaries below this mark, as they are checked >= WP_GPG40 && < WP_NUM_WEAPONS
	...
	WP_POISON_SYRINGE,		// 50
	WP_ADRENALINE_SHARE,	// 51
	WP_M97,					// 52 - shotgun
	WP_POISON_GAS,			// 53
...
#define IS_AUTORELOAD_WEAPON(weapon) \
			(	\
				...
				weapon==WP_THOMPSON	|| weapon==WP_STEN			|| weapon==WP_M97			|| \
...
// Lifetime and marker size for debug bullet paths.
#define	DEBUG_BULLET_TIME	30000

#define	DEBUG_BULLET_TICK	6
...
typedef enum {
    ...
    EV_STEP_12,
    EV_STEP_16,
#include <bgame/bg_entity_event.h.inc>
    EV_MAX_EVENTS,
} entity_event_t;

// Event types must fit in the 8-bit network field.
typedef char jm_assert_events_fit_in_etype[
    (ET_EVENTS + EV_MAX_EVENTS <= 256) ? 1 : -1 ];
...
typedef enum extWeaponStats_s
{
	...
	WS_POISON_SYRINGE,		// 22 // Jaybird: Poison Syringes
	WS_THROWING_KNIFE,		// 23 // Jaybird: Throwing knives
	WS_M97,					// 24 // Jaybird: Shotgun
	WS_POISON_GAS,			// 25
    WS_GOOMBA,              // 26
...
int		 BG_VerifyWeaponItemTables( void );
...
#include <bgame/str/public.h>

///////////////////////////////////////////////////////////////////////////////

#endif // BGAME_PUBLIC_H
