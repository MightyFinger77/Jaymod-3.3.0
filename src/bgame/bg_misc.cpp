...
ammotable_t ammoTableMP[WP_NUM_WEAPONS] = {
	...
	{	96,				1,		32,		32,		32,		3100,	DELAY_LOW,		150,	1350,	450,	MOD_STEN				},	// WP_STEN					// 10
...
// Cache the fixed weapon mappings instead of scanning bg_itemlist on every lookup.
static weapon_t	bg_ammoForWeapon[WP_NUM_WEAPONS];
static weapon_t	bg_clipForWeapon[WP_NUM_WEAPONS];
static gitem_t*	bg_itemForWeapon[WP_NUM_WEAPONS];
static qboolean	bg_weaponItemTablesBuilt = qfalse;

static void BG_BuildWeaponItemTables( void ) {
	gitem_t	*it;
	int		i;

	for ( i = 0 ; i < WP_NUM_WEAPONS ; i++ ) {
		bg_ammoForWeapon[i] = WP_NONE;
		bg_clipForWeapon[i] = WP_NONE;
		bg_itemForWeapon[i] = NULL;
	}
	...
	for ( it = bg_itemlist + 1 ; it->classname ; it++) {
		...
		if ( it->giTag < 0 || it->giTag >= WP_NUM_WEAPONS ) {
			continue;
		}
		if ( bg_itemForWeapon[it->giTag] ) {
			continue;	// Match the first-entry behavior of the old scan.
		}
		bg_ammoForWeapon[it->giTag] = (weapon_t)it->giAmmoIndex;
		bg_clipForWeapon[it->giTag] = (weapon_t)it->giClipIndex;
		bg_itemForWeapon[it->giTag] = it;
	}

	bg_weaponItemTablesBuilt = qtrue;
}
...
gitem_t	*BG_FindItemForWeapon( weapon_t weapon ) {
	if ( !bg_weaponItemTablesBuilt ) {
		BG_BuildWeaponItemTables();
	}
	if ( weapon >= 0 && weapon < WP_NUM_WEAPONS && bg_itemForWeapon[weapon] ) {
		return bg_itemForWeapon[weapon];
	}
...
Checks the cached weapon mappings against the original linear lookup.
==============
*/
int BG_VerifyWeaponItemTables( void ) {
	...
	for ( w = -64 ; w < WP_NUM_WEAPONS + 64 ; w++ ) {
		gitem_t	*it, *scanItem = NULL;
		weapon_t scanAmmo = WP_NONE, scanClip = WP_NONE;

		// Reproduce the old lookup for comparison.
		for ( it = bg_itemlist + 1 ; it->classname ; it++) {
			if ( it->giType == IT_WEAPON && it->giTag == w ) {
				scanAmmo = (weapon_t)it->giAmmoIndex;
				scanClip = (weapon_t)it->giClipIndex;
				scanItem = it;
				break;
			}
		}
		...
		{
			const weapon_t tblAmmo = BG_FindAmmoForWeapon( (weapon_t)w );
			const weapon_t tblClip = BG_FindClipForWeapon( (weapon_t)w );
			gitem_t* const tblItem = ( w >= 0 && w < WP_NUM_WEAPONS ) ? bg_itemForWeapon[w] : NULL;

			if ( tblAmmo != scanAmmo || tblClip != scanClip || tblItem != scanItem ) {
				Com_Printf( "BG_VerifyWeaponItemTables: MISMATCH weapon %i: "
					"ammo %i/%i clip %i/%i item %p/%p\n",
					w, tblAmmo, scanAmmo, tblClip, scanClip,
					(void*)tblItem, (void*)scanItem );
				bad++;
			}
		}
	}

	Com_Printf( "BG_VerifyWeaponItemTables: %i inputs checked, %i mismatches -> %s\n",
		WP_NUM_WEAPONS + 128, bad, bad ? "FAIL" : "PASS" );
	return bad;
}
...
weapon_t BG_FindClipForWeapon( weapon_t weapon) {
	...
	return bg_clipForWeapon[weapon];
}
...
weapon_t BG_FindAmmoForWeapon( weapon_t weapon ) {
	...
	return bg_ammoForWeapon[weapon];
}
