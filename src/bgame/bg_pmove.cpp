...
static qboolean PM_CheckProne (void)
{
	...
	if( !(pm->ps->eFlags & EF_PRONE) ) {
		...
		if( ((pm->ps->pm_flags & PMF_DUCKED && pm->cmd.doubleTap == DT_FORWARD) ||
			...
			pm->ps->eFlags |= EF_PRONE;
			// Keep the leg offset found by the prone clearance check.
			PM_TraceAllLegs( &trace, &pm->pmext->proneLegsOffset, pm->ps->origin, pm->ps->origin );
			pm->ps->eFlags &= ~EF_PRONE;
...
static void PM_CheckDuck (void)
{
	...
	{	// duck
		// Rate-limit repeated crouches without delaying a normal crouch or stand.
		qboolean duckAllowed = qtrue;

		if( !(pm->ps->pm_flags & PMF_DUCKED) && pm->ps->weapon != WP_MORTAR_SET ) {
			if( pm->cmd.serverTime - pm->pmext->duckRateWindow >= DUCK_RATE_WINDOW ) {
				pm->pmext->duckRateWindow = pm->cmd.serverTime;
				pm->pmext->duckRateCount = 0;
			}

			if( pm->pmext->duckRateCount >= DUCK_RATE_MAX &&
			    pm->cmd.serverTime - pm->pmext->lastDuckTime < DUCK_RATE_MIN_INTERVAL ) {
				duckAllowed = qfalse;
			} else {
				pm->pmext->duckRateCount++;
				pm->pmext->lastDuckTime = pm->cmd.serverTime;
			}
		}
	...
	for(wp=0;wp<WP_NUM_WEAPONS;wp++) {
		...
		if( COM_BitCheck( pm->ps->weapons, wp) ) {
			...
			if(pm->ps->weapHeat[wp]) {
				// Carry fractional cooling between frames so cooling is frame-rate independent.
				float coolAmount = (float)GetAmmoTableData(wp)->coolRate * pml.frametime;

				if( pm->skill[SK_HEAVY_WEAPONS] >= 2 && pm->ps->stats[STAT_PLAYER_CLASS] == PC_SOLDIER )
					coolAmount *= 2.f;

				coolAmount += pm->pmext->weapHeatFrac[wp];

				{
					int iCool = (int)coolAmount;
					pm->pmext->weapHeatFrac[wp] = coolAmount - (float)iCool;
					pm->ps->weapHeat[wp] -= iCool;
				}

				if(pm->ps->weapHeat[wp] < 0) {
					pm->ps->weapHeat[wp] = 0;
					pm->pmext->weapHeatFrac[wp] = 0.f;
				}
...
//		break;
	...
	if (wpnScale) {
		...
		// Preserve fractional spread changes and apply the server's tuning value.
		increase = cmdTime * viewchange * AIMSPREAD_INCREASE_RATE
		           * cvars::bg_aimSpreadTurnScale.fvalue;
	} else {
		increase = 0;
...
//		PM_CoolWeapons(); // Gordon: Arnout says this is how it's wanted ( bleugh ) no cooldown on weaps while using mg42, but need to update heat on mg42 itself
  ...
  if (pm->ps->weaponstate != weaponstate_last) {
	...
	if( pm->pmext->weapRecoilTime ) {
		...
		// Apply recoil on a fixed 15 ms grid so it does not vary with frame rate.
		for( i = pm->pmext->lastRecoilDeltaTime - (pm->pmext->lastRecoilDeltaTime % 15); i < deltaTime; i += 15 ) {
			if( i < pm->pmext->lastRecoilDeltaTime )
				continue;
...
void PM_UpdateLean(playerState_t *ps, usercmd_t *cmd, pmove_t *tpm) {
	...
	// This function can run outside Pmove, so use the state passed by the caller.
	if( ps->weaponDelay && ps->weapon == WP_PANZERFAUST ) {
		return;
	}
