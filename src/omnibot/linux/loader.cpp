...
Omnibot_LoadLibrary( const int version, const char* const libbase, const char* const customdir )
{
    ...
    // ETL ships omnibot_et.x86_64.so (and .aarch64.so). omnibot_et.so is 32-bit.
    vector<string> libnames;
#if defined(__x86_64__) || defined(__amd64__)
    libnames.push_back( string(libbase) + ".x86_64" + __LIB_SUFFIX );
#elif defined(__aarch64__)
    libnames.push_back( string(libbase) + ".aarch64" + __LIB_SUFFIX );
#endif
