...
//
// Must stay a trivial 4-byte standard-layout type (no user-declared
// constructors). Omni-bot 0.93 on Windows is MSVC and passes/returns
// this handle in a register. MinGW used to emit user ctors, so GCC
// passed a hidden pointer instead -- AddBot then SIGSEGV'd on the
// first GetEntity* / GAME_CLIENTCONNECTED callback.
class GameEntity
{
public:
	...
	void Reset()
	{
		// Match official Omni-bot GameEntity(): index -1, serial 0.
		udata.m_Short[0] = -1;
		udata.m_Short[1] = 0;
	}
	...
	static GameEntity From(obint16 _index, obint16 _serial)
	{
		GameEntity e;
		e.udata.m_Short[0] = _index;
		e.udata.m_Short[1] = _serial;
		return e;
	}

	// Public so the type stays C++98 POD / standard-layout. Private
	// members would make MinGW pass this by hidden pointer again.
	union udatatype
	{
		obint32			m_Int;
		obint16			m_Short[2];
...
typedef char GameEntity_size_must_be_4[sizeof(GameEntity) == 4 ? 1 : -1];
