...
// TODO:
// clarify: Event_SystemGravity d = { -g_gravity.value }; // -value ?
...
{
	const obint32 raw = _ent.AsInt();
	// 0.93 may pass a raw game id (EntityFromID(n) == n) or a packed
	// index+serial handle. Treat serial 0 as index-only.
	int index = _ent.GetIndex();
	if (_ent.GetSerial() == 0 && raw >= 0 && raw < MAX_GENTITIES)
		index = raw;
	if(index < 0 || index >= MAX_GENTITIES)
		return NULL;
	if(index == ENTITYNUM_WORLD)
		return &g_entities[ENTITYNUM_WORLD];
	if(!g_entities[index].inuse)
		return NULL;
	if(_ent.GetSerial() != 0 && m_EntityHandles[index].m_HandleSerial != _ent.GetSerial())
		return NULL;
...
{
	if(!_ent)
		return GameEntity::Invalid();
	const int index = _ent - g_entities;
	if(index < 0 || index >= MAX_GENTITIES)
		return GameEntity::Invalid();
	return GameEntity::From(index, m_EntityHandles[index].m_HandleSerial);
}
...
void AddDeferredGoal(gentity_t *ent)
{
	...
	{
		G_Printf(S_COLOR_YELLOW "OMNIBOT: deferred goal buffer full, skipping entity %d\n",
...
public:
	...
	{
		G_Printf("OMNIBOT: AddBot enter helper=%p id=%d size=%u payload=%p\n",
			(const void *)&_data, _data.GetMessageId(), (unsigned)_data.GetBlockSize(), _data.GetVoid());
		OB_GETMSG(Msg_Addbot);
		if (!pMsg)
		{
			PrintError("Could not add bot!");
			G_Printf(S_COLOR_RED "OMNIBOT: AddBot rejected: missing Msg_Addbot (interface mismatch with omnibot_et_x64).\n");
			return -1;
		}

		// Let the engine pick a public slot. Requesting a specific slot that
		// sits inside sv_privateClients makes ETL's SV_BotAllocateClient
		// ignore the request and can leave the game/engine client lists
		// out of sync on x64.
		...
		char botname[64];
		Q_strncpyz(botname, pMsg->m_Name, sizeof(botname));
		if (!botname[0])
			Q_strncpyz(botname, va("OmniBot%i", num), sizeof(botname));
		...
		// Mark as bot before ClientConnect so Jaymod user/ban/userinfo
		// paths do not treat the slot as a listen-server localhost host.
		bot->s.number = num;
		bot->r.svFlags |= SVF_BOT;

		trap_SetUserinfo(num, userinfo);

		G_Printf("OMNIBOT: AddBot slot %d name '%s'\n", num, botname);

		string connectErrMsg;
		bool denied = false;
		try
		{
			denied = ClientConnect(connectErrMsg, num, qtrue, qtrue);
		}
		catch (...)
		{
			G_Printf(S_COLOR_RED "OMNIBOT: ClientConnect threw adding bot slot %d. Slot freed, server kept running.\n", num);
			trap_BotFreeClient(num);
			bot->r.svFlags &= ~SVF_BOT;
			bot->inuse = qfalse;
			return -1;
		}
		...
		if (!bot->inuse)
		{
			G_Printf(S_COLOR_RED "OMNIBOT: ClientConnect accepted slot %d but entity is not in use. Slot freed.\n", num);
			trap_BotFreeClient(num);
			bot->r.svFlags &= ~SVF_BOT;
			return -1;
		}
	...
	void RemoveBot(const MessageHelper &_data)
	{
		...
		if (!pMsg)
		{
			G_Printf(S_COLOR_RED "OMNIBOT: RemoveBot rejected: missing Msg_Kickbot.\n");
			return;
		}
	...
	obResult ChangeTeam(int _client, int _newteam, const MessageHelper *_data)
	{
		...
		if (_client < 0 || _client >= MAX_CLIENTS) {
			G_Printf(S_COLOR_RED "OMNIBOT: ChangeTeam bad slot %d\n", _client);
			return InvalidEntity;
		}
		gentity_t* bot = &g_entities[_client];
		if (!bot->client) {
			G_Printf(S_COLOR_RED "OMNIBOT: ChangeTeam slot %d has no client\n", _client);
			return InvalidEntity;
		}
		...
		G_Printf("OMNIBOT: ChangeTeam slot %d -> %s\n", _client, teamName);
	...
	{
		if (_client < 0 || _client >= MAX_CLIENTS) {
			G_Printf(S_COLOR_RED "OMNIBOT: ChangeClass bad slot %d\n", _client);
			return InvalidEntity;
		}
		gentity_t* bot = &g_entities[_client];
		if (!bot->client) {
			G_Printf(S_COLOR_RED "OMNIBOT: ChangeClass slot %d has no client\n", _client);
			return InvalidEntity;
		}
	...
	GameEntity EntityFromID(const int _gameId)
	{
		...
		return pEnt ? HandleFromEntity(pEnt) : GameEntity::Invalid();
	}
...
#if defined(_WIN64)
// omnibot_et_x64.dll is MSVC. MinGW GCC uses the Itanium C++ ABI
// (two destructor vtable slots, different non-POD returns). Passing a
// GCC IEngineInterface* into pfnInitialize makes the first AddBot /
// GetEntity* call jump to the wrong slot or smash the stack.
//
// ETL avoids this by compiling qagame with MSVC. We keep MinGW and
// expose a hand-built MSVC vtable (declaration order, one dtor slot).
static ETInterface *g_etImpl = 0;
static const char *g_obThunkName = "pfnUpdate";
#define OB_THUNK(n) do { g_obThunkName = (n); } while (0)

static bool ob_mem_ok(uintptr_t p, SIZE_T bytes, bool write)
{
	if (p < 0x10000ull || p > 0x00007FFFFFFEFFFFull)
		return false;
	MEMORY_BASIC_INFORMATION mbi;
	if (VirtualQuery((const void *)p, &mbi, sizeof(mbi)) != sizeof(mbi))
		return false;
	if (mbi.State != MEM_COMMIT || (mbi.Protect & PAGE_GUARD))
		return false;
	const uintptr_t end = p + bytes;
	if (end < p || end > (uintptr_t)mbi.BaseAddress + mbi.RegionSize)
		return false;
...
// Omni-bot 0.93 GameEntity has user constructors. MSVC x64 may pass the
// 4-byte handle in a register or as a pointer to a temporary.
static GameEntity ge_bits(intptr_t bits)
{
	GameEntity e;
	const uintptr_t p = (uintptr_t)bits;
	if (p > 0xFFFFFFFFull || ob_mem_ok(p, 4, false)) {
		e.FromInt(*reinterpret_cast<const int *>(p));
		return e;
	}
	e.FromInt((int)bits);
	return e;
...
// Official 0.8 GameEntity is a 4-byte class with a user ctor (no virtuals).
// MSVC x64 therefore uses a hidden return pointer and, after constructing
// the object there, "puts the address of the return object in RAX".
// Returning the handle bits in EAX made omnibot_et_x64 deref 0 / 65535 /
// 65536 — that is the 0xc0000005 after every successful *sret store.
static void *store_ge(int *sret, int bits)
{
	if ((uintptr_t)sret >= 0x10000ull) {
		*sret = bits;
		return sret;
	}
...
static int msvc_AddBot(void *, intptr_t dataBits)
{
	OB_THUNK("AddBot");
	if (!ob_mem_ok((uintptr_t)dataBits, 16, false)) {
		G_Printf(S_COLOR_RED "OMNIBOT: AddBot bad MessageHelper %p\n", (void *)dataBits);
		return -1;
	}
	return g_etImpl->AddBot(*reinterpret_cast<const MessageHelper *>(dataBits));
}
static void msvc_RemoveBot(void *, intptr_t dataBits)
{
	OB_THUNK("RemoveBot");
	if (!ob_mem_ok((uintptr_t)dataBits, 16, false))
		return;
	g_etImpl->RemoveBot(*reinterpret_cast<const MessageHelper *>(dataBits));
}
static obResult msvc_ChangeTeam(void *, int client, int team, const MessageHelper *data) { OB_THUNK("ChangeTeam"); return g_etImpl->ChangeTeam(client, team, data); }
static obResult msvc_ChangeClass(void *, int client, int cls, const MessageHelper *data) { OB_THUNK("ChangeClass"); return g_etImpl->ChangeClass(client, cls, data); }
static void msvc_UpdateBotInput(void *, int client, const ClientInput &input) { OB_THUNK("UpdateBotInput"); g_etImpl->UpdateBotInput(client, input); }
static void msvc_BotCommand(void *, int client, const char *cmd) { OB_THUNK("BotCommand"); g_etImpl->BotCommand(client, cmd); }
static obBool msvc_IsInPVS(void *, const float pos[3], const float target[3]) { OB_THUNK("IsInPVS"); return g_etImpl->IsInPVS(pos, target); }
static obResult msvc_TraceLine(void *, obTraceResult &result, const float start[3], const float end[3], const AABB *box, int mask, int user, obBool usePVS) { OB_THUNK("TraceLine"); return g_etImpl->TraceLine(result, start, end, box, mask, user, usePVS); }
static int msvc_GetPointContents(void *, const float pos[3]) { OB_THUNK("GetPointContents"); return g_etImpl->GetPointContents(pos); }
static void *msvc_GetLocalGameEntity(void *, int *sret)
{
	OB_THUNK("GetLocalGameEntity");
	return store_ge(sret, g_etImpl->GetLocalGameEntity().AsInt());
}
static void *msvc_FindEntityInSphere(void *, int *sret, const float pos[3], float radius, intptr_t startBits, int classId)
{
	OB_THUNK("FindEntityInSphere");
	return store_ge(sret, g_etImpl->FindEntityInSphere(pos, radius, ge_bits(startBits), classId).AsInt());
}
static int msvc_GetEntityClass(void *, intptr_t entBits) { OB_THUNK("GetEntityClass"); return g_etImpl->GetEntityClass(ge_bits(entBits)); }
static obResult msvc_GetEntityCategory(void *, intptr_t entBits, BitFlag32 &category) { OB_THUNK("GetEntityCategory"); return g_etImpl->GetEntityCategory(ge_bits(entBits), category); }
static obResult msvc_GetEntityFlags(void *, intptr_t entBits, BitFlag64 &flags) { OB_THUNK("GetEntityFlags"); return g_etImpl->GetEntityFlags(ge_bits(entBits), flags); }
static obResult msvc_GetEntityPowerups(void *, intptr_t entBits, BitFlag64 &flags) { OB_THUNK("GetEntityPowerups"); return g_etImpl->GetEntityPowerups(ge_bits(entBits), flags); }
static obResult msvc_GetEntityEyePosition(void *, intptr_t entBits, float pos[3]) { OB_THUNK("GetEntityEyePosition"); return g_etImpl->GetEntityEyePosition(ge_bits(entBits), pos); }
static obResult msvc_GetEntityBonePosition(void *, intptr_t entBits, int boneid, float pos[3]) { OB_THUNK("GetEntityBonePosition"); return g_etImpl->GetEntityBonePosition(ge_bits(entBits), boneid, pos); }
static obResult msvc_GetEntityOrientation(void *, intptr_t entBits, float fwd[3], float right[3], float up[3]) { OB_THUNK("GetEntityOrientation"); return g_etImpl->GetEntityOrientation(ge_bits(entBits), fwd, right, up); }
static obResult msvc_GetEntityVelocity(void *, intptr_t entBits, float velocity[3]) { OB_THUNK("GetEntityVelocity"); return g_etImpl->GetEntityVelocity(ge_bits(entBits), velocity); }
static obResult msvc_GetEntityPosition(void *, intptr_t entBits, float pos[3]) { OB_THUNK("GetEntityPosition"); return g_etImpl->GetEntityPosition(ge_bits(entBits), pos); }
static obResult msvc_GetEntityLocalAABB(void *, intptr_t entBits, AABB &aabb) { OB_THUNK("GetEntityLocalAABB"); return g_etImpl->GetEntityLocalAABB(ge_bits(entBits), aabb); }
static obResult msvc_GetEntityWorldAABB(void *, intptr_t entBits, AABB &aabb) { OB_THUNK("GetEntityWorldAABB"); return g_etImpl->GetEntityWorldAABB(ge_bits(entBits), aabb); }
static obResult msvc_GetEntityWorldOBB(void *, intptr_t entBits, float *center, float *axis0, float *axis1, float *axis2, float *extents) { OB_THUNK("GetEntityWorldOBB"); return g_etImpl->GetEntityWorldOBB(ge_bits(entBits), center, axis0, axis1, axis2, extents); }
static obResult msvc_GetEntityGroundEntity(void *, intptr_t entBits, GameEntity &moveent) { OB_THUNK("GetEntityGroundEntity"); return g_etImpl->GetEntityGroundEntity(ge_bits(entBits), moveent); }
static void *msvc_GetEntityOwner(void *, int *sret, intptr_t entBits)
{
	OB_THUNK("GetEntityOwner");
	return store_ge(sret, g_etImpl->GetEntityOwner(ge_bits(entBits)).AsInt());
}
static int msvc_GetEntityTeam(void *, intptr_t entBits) { OB_THUNK("GetEntityTeam"); return g_etImpl->GetEntityTeam(ge_bits(entBits)); }
static const char *msvc_GetEntityName(void *, intptr_t entBits) { OB_THUNK("GetEntityName"); return g_etImpl->GetEntityName(ge_bits(entBits)); }
static obResult msvc_GetCurrentWeaponClip(void *, intptr_t entBits, FireMode mode, int &curclip, int &maxclip) { OB_THUNK("GetCurrentWeaponClip"); return g_etImpl->GetCurrentWeaponClip(ge_bits(entBits), mode, curclip, maxclip); }
static obResult msvc_GetCurrentAmmo(void *, intptr_t entBits, int weaponId, FireMode mode, int &cur, int &max) { OB_THUNK("GetCurrentAmmo"); return g_etImpl->GetCurrentAmmo(ge_bits(entBits), weaponId, mode, cur, max); }
static int msvc_GetGameTime(void *) { OB_THUNK("GetGameTime"); return g_etImpl->GetGameTime(); }
static void msvc_GetGoals(void *) { OB_THUNK("GetGoals"); g_etImpl->GetGoals(); }
static void msvc_GetPlayerInfo(void *, intptr_t infoBits)
{
	OB_THUNK("GetPlayerInfo");
	if (!ob_mem_ok((uintptr_t)infoBits, sizeof(obPlayerInfo), true)) {
		G_Printf(S_COLOR_RED "OMNIBOT: GetPlayerInfo bad dest %p\n", (void *)infoBits);
		return;
	}
	g_etImpl->GetPlayerInfo(*reinterpret_cast<obPlayerInfo *>(infoBits));
}
static obResult msvc_InterfaceSendMessage(void *, intptr_t dataBits, intptr_t entBits)
{
	OB_THUNK("InterfaceSendMessage");
	if (!ob_mem_ok((uintptr_t)dataBits, 16, false))
		return InvalidParameter;
	return g_etImpl->InterfaceSendMessage(*reinterpret_cast<const MessageHelper *>(dataBits), ge_bits(entBits));
}
static bool msvc_DebugLine(void *, const float start[3], const float end[3], const obColor &color, float time) { OB_THUNK("DebugLine"); return g_etImpl->DebugLine(start, end, color, time); }
static bool msvc_DebugBox(void *, const float mins[3], const float maxs[3], const obColor &color, float time) { OB_THUNK("DebugBox"); return g_etImpl->DebugBox(mins, maxs, color, time); }
static bool msvc_DebugArrow(void *, const float start[3], const float end[3], const obColor &color, float time) { OB_THUNK("DebugArrow"); return g_etImpl->DebugArrow(start, end, color, time); }
static bool msvc_DebugRadius(void *, const float pos[3], const float radius, const obColor &color, float time) { OB_THUNK("DebugRadius"); return g_etImpl->DebugRadius(pos, radius, color, time); }
static bool msvc_DebugPolygon(void *, const obVec3 *verts, const int numverts, const obColor &color, float time, int flags) { OB_THUNK("DebugPolygon"); return g_etImpl->DebugPolygon(verts, numverts, color, time, flags); }
static bool msvc_PrintScreenText(void *, const float pos[3], float duration, const obColor &color, const char *msg) { OB_THUNK("PrintScreenText"); return g_etImpl->PrintScreenText(pos, duration, color, msg); }
static void msvc_PrintError(void *, const char *error) { OB_THUNK("PrintError"); g_etImpl->PrintError(error); }
static void msvc_PrintMessage(void *, const char *msg) { OB_THUNK("PrintMessage"); g_etImpl->PrintMessage(msg); }
static const char *msvc_GetMapName(void *) { OB_THUNK("GetMapName"); return g_etImpl->GetMapName(); }
static void msvc_GetMapExtents(void *, AABB &aabb) { OB_THUNK("GetMapExtents"); g_etImpl->GetMapExtents(aabb); }
static void *msvc_EntityFromID(void *, int *sret, int gameId)
{
	OB_THUNK("EntityFromID");
	const int v = g_etImpl->EntityFromID(gameId).AsInt();
	void *const rax = store_ge(sret, v);
	static int logged;
	if (logged < 4) {
		G_Printf("OMNIBOT: EntityFromID id=%d bits=%d rax=%p\n", gameId, v, rax);
		++logged;
	}
	return rax;
}
static void *msvc_EntityByName(void *, int *sret, const char *name)
{
	OB_THUNK("EntityByName");
	return store_ge(sret, g_etImpl->EntityByName(name).AsInt());
}
static int msvc_IDFromEntity(void *, intptr_t entBits) { OB_THUNK("IDFromEntity"); return g_etImpl->IDFromEntity(ge_bits(entBits)); }
static bool msvc_DoesEntityStillExist(void *, intptr_t hndl) {
	OB_THUNK("DoesEntityStillExist");
	GameEntity e = ge_bits(hndl);
	return g_etImpl->DoesEntityStillExist(e);
}
static int msvc_GetAutoNavFeatures(void *, AutoNavFeature *feature, int max) { return g_etImpl->GetAutoNavFeatures(feature, max); }
static const char *msvc_GetGameName(void *) { return g_etImpl->GetGameName(); }
static const char *msvc_GetModName(void *) { return g_etImpl->GetModName(); }
static const char *msvc_GetModVers(void *) { return g_etImpl->GetModVers(); }
static const char *msvc_GetBotPath(void *) { return g_etImpl->GetBotPath(); }
static const char *msvc_GetLogPath(void *) { return g_etImpl->GetLogPath(); }
static void msvc_Dtor(void *) {}

static void *g_msvcVtable[] = {
	(void *)msvc_AddBot,
	(void *)msvc_RemoveBot,
	(void *)msvc_ChangeTeam,
	(void *)msvc_ChangeClass,
	(void *)msvc_UpdateBotInput,
	(void *)msvc_BotCommand,
	(void *)msvc_IsInPVS,
	(void *)msvc_TraceLine,
	(void *)msvc_GetPointContents,
	(void *)msvc_GetLocalGameEntity,
	(void *)msvc_FindEntityInSphere,
	(void *)msvc_GetEntityClass,
	(void *)msvc_GetEntityCategory,
	(void *)msvc_GetEntityFlags,
	(void *)msvc_GetEntityPowerups,
	(void *)msvc_GetEntityEyePosition,
	(void *)msvc_GetEntityBonePosition,
	(void *)msvc_GetEntityOrientation,
	(void *)msvc_GetEntityVelocity,
	(void *)msvc_GetEntityPosition,
	(void *)msvc_GetEntityLocalAABB,
	(void *)msvc_GetEntityWorldAABB,
	(void *)msvc_GetEntityWorldOBB,
	(void *)msvc_GetEntityGroundEntity,
	(void *)msvc_GetEntityOwner,
	(void *)msvc_GetEntityTeam,
	(void *)msvc_GetEntityName,
	(void *)msvc_GetCurrentWeaponClip,
	(void *)msvc_GetCurrentAmmo,
	(void *)msvc_GetGameTime,
	(void *)msvc_GetGoals,
	(void *)msvc_GetPlayerInfo,
	(void *)msvc_InterfaceSendMessage,
	(void *)msvc_DebugLine,
	(void *)msvc_DebugBox,
	(void *)msvc_DebugArrow,
	(void *)msvc_DebugRadius,
	(void *)msvc_DebugPolygon,
	(void *)msvc_PrintScreenText,
	(void *)msvc_PrintError,
	(void *)msvc_PrintMessage,
	(void *)msvc_GetMapName,
	(void *)msvc_GetMapExtents,
	(void *)msvc_EntityFromID,
	(void *)msvc_EntityByName,
	(void *)msvc_IDFromEntity,
	(void *)msvc_DoesEntityStillExist,
	(void *)msvc_GetAutoNavFeatures,
	(void *)msvc_GetGameName,
	(void *)msvc_GetModName,
	(void *)msvc_GetModVers,
	(void *)msvc_GetBotPath,
	(void *)msvc_GetLogPath,
	(void *)msvc_Dtor,
};

struct MsvcEngineObject
{
	void **vptr;
};

static MsvcEngineObject g_msvcEngine;
#endif

IEngineInterface *Bot_GetBotVisibleInterface()
{
#if defined(_WIN64)
	return reinterpret_cast<IEngineInterface *>(&g_msvcEngine);
#else
	return g_InterfaceFunctions;
#endif
}
...
int Bot_Interface_Init()
{
	...
	g_msvcEngine.vptr = g_msvcVtable;
	G_Printf("OMNIBOT: Win64 MSVC-layout engine vtable for omnibot_et_x64 (MinGW qagame).\n");
#endif
...
//#endif
		...
		// C++ try/catch does not catch Windows ACCESS_VIOLATION from the
		// MSVC Omni-bot DLL or a bad vtable thunk. VEH + setjmp keeps
		// etlded up and stops further bot ticks after the first native fault.
#if defined(_WIN32)
		if (!g_obUpdateDisabled)
		{
			OmnibotEnsureVeh();
			if (setjmp(g_obJmp) != 0)
			{
				...
				G_Printf(S_COLOR_RED "OMNIBOT: native fault 0x%08lx in %s. Further bot updates disabled. Server kept running.\n",
					(unsigned long)g_obLastCode, g_obThunkName ? g_obThunkName : "pfnUpdate");
			}
			else
			{
#if defined(_WIN64)
				g_obThunkName = "pfnUpdate";
#endif
		...
		{
			G_Printf(S_COLOR_RED "OMNIBOT: pfnUpdate threw (AddBot / interface). Server kept running.\n");
		}
#endif
		//////////////////////////////////////////////////////////////////////////
	}
...
void Bot_Event_ClientConnected(int _client, qboolean _isbot)
{
	if(IsOmnibotLoaded())
	{
		...
		{
			G_Printf(S_COLOR_RED "OMNIBOT: GAME_CLIENTCONNECTED threw for slot %d. Server kept running.\n", _client);
		}
	}
}
...
void Bot_Event_EntityDeleted(gentity_t *pEnt)
{
	if(pEnt)
	{
		...
		if(IsOmnibotLoaded())
		{
			Event_EntityDeleted d = { HandleFromEntity(pEnt) };
